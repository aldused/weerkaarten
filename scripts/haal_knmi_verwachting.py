...
def scrape():
    ...
    if table:
        for day_div in table.find_all("div", class_="weather-forecast__day"):
            ...
            # Waarschuwingen staan bij KNMI los van het weersymbool. Zonder
            # deze velden ziet een waarschuwing-icoon er op Weerlab alleen
            # grijs uit en ontbreekt de reden (bijv. hitte of onweer).
            code_class = next(
                (c for c in day_div.get("class", []) if c.startswith("weather-forecast__code-")),
                "",
            )
            code_key = code_class.removeprefix("weather-forecast__code-")
            code_nl = {"yellow": "geel", "orange": "oranje", "red": "rood"}.get(code_key, "")
            heeft_waarschuwing = code_key in {"yellow", "orange", "red", "warning"}
            waarschuwingen = []
            for el in day_div.select(".weather-forecast__warning-messages span"):
                melding = el.get_text(" ", strip=True)
                if melding and melding not in waarschuwingen:
                    waarschuwingen.append(melding)
            if heeft_waarschuwing or waarschuwingen:
                dag["waarschuwing"] = {
                    "status": "mogelijk" if code_key == "warning" else "actief",
                    "code": code_nl,
                    "fenomenen": waarschuwingen,
                }
