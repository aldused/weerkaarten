...
print("MOSMIX ophalen (max temp dag 06-18u België)...")
data_per_day = {}
...
for day, dag_data in data_per_day.items():
    ...
    ax_h.text(0.012, 0.22, "Max temp dag (06–18u)  ·  MOS ECMWF/ICON", fontsize=8,
    ...
    for naam, tx_v in dag_data.items():
        if naam not in coords: continue
        lon, lat = coords[naam]
        ax.text(lon, lat + 0.03, f"{tx_v:.1f}",
    ...
    leg = ax.inset_axes([0.01, 0.02, 0.15, 0.09])
    ...
    leg.add_patch(plt.Rectangle((0.05,0.18),0.16,0.64,facecolor="#cc2200",
                  transform=leg.transAxes,zorder=1))
    leg.text(0.27, 0.50, "Max temp dag (06–18u)", fontsize=5, va="center", transform=leg.transAxes)

    ax.text(1.0, 0.0, f"© Ed Aldus | Data: DWD (MOSMIX) | {now_str2}",
            transform=ax.transAxes, fontsize=7, style="italic",
            ha="right", va="bottom", color="#555555")

    fname = f"kaart_be_temp_dag_{dag_nl.lower()}_{day.strftime('%d%b%Y').lower()}.png"
