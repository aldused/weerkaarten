#!/usr/bin/env python3
"""
Météo-France AROME via Open-Meteo naar het canvas-formaat van harmonie_canvas.html.

Gebruik:
    python3 scripts/arome_fr_update.py
    python3 scripts/arome_fr_update.py --grid-step 0.05 --days 3
"""
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--prefix", default="arome_fr")
    p.add_argument("--model", default="meteofrance_arome_france")
    p.add_argument("--days", type=int, default=3)
    p.add_argument("--grid-step", type=float, default=0.1)
    p.add_argument("--batch-size", type=int, default=80)
    p.add_argument("--lon-min", type=float, default=0.5)
    p.add_argument("--lon-max", type=float, default=11.3)
    p.add_argument("--lat-min", type=float, default=49.0)
...
def post_bulk(batch: list[tuple[int, int, float, float]], args: argparse.Namespace) -> list[dict] | None:
    """Returns list of location dicts, or None if entire batch is out of model domain."""
    key = _load_key()
    ...
    for attempt in range(4):
        try:
            ...
            if r.status_code == 400 and "No data is available" in r.text:
                return None  # buiten modeldomein
    ...
    raise RuntimeError(f"Open-Meteo AROME call mislukt: {last_err}")
...
def main() -> int:
    ...
    print(f"AROME France: {n_lat}x{n_lon} = {len(coords)} punten, {len(batches)} batches")
    ...
    # AROME heeft alleen cloud_cover totaal (geen hoog/mid/laag)
    # Gebruik totale bewolking voor alle drie componenten
    cloud = (arrays["cloud_cover"] / 100.0).astype(np.float32)
    ...
    write_bin(WORK_DIR / f"{prefix}_data_bewolking.bin", (cloud, cloud, cloud))
    ...
    meta = {
        "model": f"Météo-France AROME via Open-Meteo ({args.model})",
        ...
        "parameters": {
            ...
            "bewolking": {"file": f"{prefix}_data_bewolking.bin", "components": 3, "label": "Bewolking (totaal)"},
            "wind": {"file": f"{prefix}_data_wind.bin", "components": 2, "label": "Wind 10m (Bft)"},
            "windstoten": {"file": f"{prefix}_data_windstoten.bin", "components": 2, "label": "Windstoten 10m (Bft)"},
    ...
    # Upload naar R2 (zelfde bucket als harmonie/ecmwf)
    try:
        import boto3
        s3 = boto3.client(
            "s3",
