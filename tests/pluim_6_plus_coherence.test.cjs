...
cloudGaps.hourly.cloud_cover_low_member17[2]=null;
const shadow=context.buildModel(cloudGaps,null,start,Date.parse(times.at(-1))).panels[0];
assert.deepEqual(Array.from(shadow.times,t=>t.toISOString()),[times[0],times[2],times[3]]);
assert.deepEqual(Array.from(shadow.stacked.validCounts),[51,50,51]);
for(const percentages of shadow.stacked.percentagesByTime)assert.equal(percentages.reduce((a,b)=>a+b,0),100);
assert.match(source,/panel\?\.times \|\| activeModel\?\.times/,'tooltip uses the native cloud time axis');
assert.match(source,/const legItems = unavailable \? \[\]/,'no fanplot legend for unavailable data');
