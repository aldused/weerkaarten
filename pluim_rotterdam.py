"""
pluim_rotterdam.py — Ensemble pluim Rotterdam
4 panelen: temperatuur / neerslag+kans / sneeuw+hagel kans / wind+stoten
Lokaal gebruik
"""
...
def haal_ensemble():
    url = (
        ...
        "&hourly=temperature_2m,precipitation,snowfall,showers,windspeed_10m,windgusts_10m"
        "&models=ecmwf_ifs025"
...
# Sneeuw/hagel kans combineren
kans_snow_hagel = np.sum((alle_snow > 0.0) | (alle_shower > 0.1), axis=0) / n_leden * 100
med_wind  = np.median(alle_wind, axis=0)
p75_wind  = np.percentile(alle_wind, 75, axis=0)
...
gs  = gridspec.GridSpec(4, 1, figure=fig,
                        height_ratios=[1, 0.55, 0.55, 0.7], hspace=0.12)
...
header_ax.text(0.988,0.65,"Ensemble pluim – Rotterdam",
...
dag_lijnen(ax1); ax1.set_xticklabels([])
...
dag_lijnen(ax2); ax2.set_xticklabels([])
...
# ── Paneel 3: Sneeuw + buien/hagel kans ──
ax3 = fig.add_subplot(gs[2])
ax3.set_facecolor("#f8f9fa")
ax3.grid(axis="y",color="#e0e0e0",linewidth=0.6,zorder=0)
for spine in ["top","right"]: ax3.spines[spine].set_visible(False)
# Sneeuw/hagel kans als blauwe balkjes
ax3.bar(x, kans_snow_hagel, color="#5599dd", width=1.0, alpha=0.7, zorder=3, label="Kans sneeuw/hagel")
dag_lijnen(ax3); ax3.set_xticklabels([])
ax3.set_ylim(0, 105)
ax3.set_ylabel("Kans (%)", fontsize=9, color="#444444")
ax3.tick_params(axis="y", labelsize=8.5, colors="#444444")
ax3.set_yticks([0,20,40,60,80,100])
ax3.set_title("Kans op sneeuw/hagel", fontsize=11, color="#333", loc="left", pad=4, fontweight="bold")
ax3.legend(loc="upper right", fontsize=8, framealpha=0.9, edgecolor="#cccccc", ncol=1)

# ── Paneel 4: Wind + stoten ──
ax4 = fig.add_subplot(gs[3])
ax4.set_facecolor("#f8f9fa")
ax4.grid(axis="y",color="#e0e0e0",linewidth=0.6,zorder=0)
for spine in ["top","right"]: ax4.spines[spine].set_visible(False)
ax4.fill_between(x, med_wind, p75_stoot, color="#003366", alpha=0.10, zorder=2)
for lid in alle_wind:
    ax4.plot(x, lid, color="#27ae60", linewidth=0.5, alpha=0.25, zorder=2)
ax4.plot(x, med_wind,  color="#003366", linewidth=2.0, zorder=5, label="Wind mediaan")
ax4.plot(x, med_stoot, color="#cc6600", linewidth=1.5, zorder=5, label="Stoten mediaan")
ax4.plot(x, p75_stoot, color="#cc2200", linewidth=1.5, linestyle="--", zorder=6, label="Stoten p75")
for ms, lbl in [(19,"Bft 3"),(29,"Bft 4"),(39,"Bft 5"),(50,"Bft 6"),(62,"Bft 7")]:
    ax4.axhline(ms, color="#aaaaaa", linewidth=0.6, linestyle=":", zorder=1)
    ax4.text(1, ms+0.5, lbl, fontsize=6.5, color="#888888", ha="left", va="bottom")
dag_lijnen(ax4)
ax4.set_xticklabels(tick_lbl, fontsize=8.5, color="#444444")
ax4.set_ylim(bottom=0)
ax4.set_ylabel("Wind (km/u)", fontsize=9, color="#444444")
ax4.tick_params(axis="y", labelsize=8.5, colors="#444444")
ax4.set_title("Wind + windstoten", fontsize=11, color="#333", loc="left", pad=4, fontweight="bold")
ax4.legend(loc="upper right", fontsize=8, framealpha=0.9, edgecolor="#cccccc", ncol=3)
