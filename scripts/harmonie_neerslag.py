...
# ── Kleurenschaal neerslag (KNMI/Buienradar radar-stijl) ─────────────────────
# Blauw → groen → geel → oranje → rood → paars → roze  (0–100+ mm/u)
NEERSLAG_LEVELS = [0.1, 0.3, 0.5, 1, 2, 3, 5, 7, 10, 15, 25, 40, 50, 75, 100]
NEERSLAG_COLORS = [
    "#e0f3fb",  # 0.1   heel licht blauw
    "#b0d9f2",  # 0.3   licht blauw
    "#74b9e8",  # 0.5   hemelsblauw
    "#3a8fd4",  # 1     blauw
    "#1460b0",  # 2     donkerblauw
    "#44bb22",  # 3     groen
    "#aadd00",  # 5     geelgroen
    "#ffee00",  # 7     geel
    "#ffaa00",  # 10    oranje
    "#ff5500",  # 15    donkeroranje
    "#dd0000",  # 25    rood
    "#990000",  # 40    donkerrood
    "#770088",  # 50    paars
    "#cc44cc",  # 75    lichtpaars
    "#ffaacc",  # 100   roze
]
...
def _teken_neerslag_legenda(ax_leg):
    """Detailrijke neerslag patch-legenda met categorie-aanduidingen."""
    ax_leg.set_position([0.02, 0.022, 0.96, 0.060])
    ax_leg.clear()
    ax_leg.axis("off")
    ax_leg.set_xlim(0, 1)
    ax_leg.set_ylim(0, 1)

    # Labels per band: grens-links van elke kleurzone
    band_labels = ["0.1", "0.3", "0.5", "1", "2", "3", "5", "7", "10", "15", "25", "40", "50", "75", ">100"]
    band_colors = NEERSLAG_COLORS  # 15 kleuren
    ...
    for i, (col, lbl) in enumerate(zip(band_colors, band_labels)):
        x = x0 + i * pw
        # Kleurvak (bovenste 55% van ax_leg)
        ax_leg.add_patch(plt.Rectangle(
            (x, 0.38), pw, 0.57,
            facecolor=col, edgecolor="white", linewidth=0.5,
            transform=ax_leg.transAxes, clip_on=True
        ))
        # mm-label onder kleurvak
        ax_leg.text(x + pw / 2, 0.22, lbl, fontsize=6.2,
                    ha="center", va="center", transform=ax_leg.transAxes,
                    color="#222222", weight="bold")

    # Categorie-labels boven kleurvakken
    # Niveaus: 0.1 0.3 0.5 1 | 2 3 5 7 | 10 15 25 | 40 50 | 75 >100
    cats = [
        # (center_fraction, tekst, van_idx, tot_idx)
        (1.5 / n, "licht",   0, 4),   # 0.1–2 mm/u
        (5.5 / n, "matig",   4, 8),   # 2–10 mm/u
        (9.5 / n, "zwaar",   8, 11),  # 10–40 mm/u
        (12.5 / n, "hevig", 11, 13),  # 40–75 mm/u
        (14.0 / n, "extm",  13, 15),  # 75+ mm/u
    ]
    for cx, tekst, van, tot in cats:
        x_van = van * pw
        x_tot = tot * pw
        # Lijn boven de betrokken vakken
        ax_leg.plot([x_van + 0.003, x_tot - 0.003], [0.97, 0.97],
                    color="#555555", linewidth=0.8, transform=ax_leg.transAxes)
        ax_leg.text(cx, 0.88, tekst, fontsize=5.8, ha="center", va="center",
                    transform=ax_leg.transAxes, color="#333333")

    # Eenheid-label links onderin
    ax_leg.text(0.5, 0.04, "Neerslagintensiteit (mm/u)", fontsize=7.5,
                ha="center", va="bottom", transform=ax_leg.transAxes, color="#444444")


def render_kaart(lats, lons, precip_2d, tijdstip_str, uur_idx, run_str, output_dir="."):
    """Render neerslagkaart zonder gridwaarden, met gedetailleerde patch-legenda."""
    try:
    ...
    _teken_neerslag_legenda(ax_leg)
