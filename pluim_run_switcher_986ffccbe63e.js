(function () {
  ...
  const state = {
    ...
    locationFallbackHour: null,
    // Alleen een zelfgekozen cyclus verdient een waarschuwing als de plaats hem
    // niet kan leveren; de standaardkeuze is de nieuwste run en mag stil wijken.
    runChosenByUser: requestedHour != null,
  ...
  function archiveManifestReferenceMs() {
    const manifest = state.archiveManifest;
    if (!manifest || manifest.complete !== true || !Array.isArray(manifest.runs)) return NaN;
    const values = manifest.runs
      .filter(item => item && item.complete === true
        && Number(item.station_count ?? manifest.station_count) === stations.length
        && Number(item.member_count ?? manifest.member_count) === 51)
      .map(item => Date.parse(item.run || ''))
      .filter(Number.isFinite);
    return values.length ? Math.max(...values) : NaN;
  }

  function latestReferenceMs() {
    // Een stationarchief alleen is nooit een publicatiesignaal. Het complete
    // archiefmanifest is dat wel: zowel directe GRIB-data als de vroege ECPDS-
    // puntdata wordt pas daarin zichtbaar nadat alle 39 stations zijn gepubliceerd.
    const values = [
      metaRunMs(state.liveMeta),
      metaRunMs(state.directMeta),
      archiveManifestReferenceMs(),
    ]
      .filter(Number.isFinite);
    return values.length ? Math.max(...values) : NaN;
  }
  ...
  function isEarlyArchiveRun(run) {
    return !!run && run.source && run.source.access === 'ecmwf_prescheduled_point_api';
  }
  ...
  function publishedRunAllows(run, variables = requiredVariables) {
    ...
    // De vroege ECPDS-producent publiceert eveneens stationbestanden vóór het
    // manifest. Zonder matching complete entry is zo'n run dus nog niet atomair.
    if (isEarlyArchiveRun(run)) return false;
  ...
  function unitFor(variable, url) {
    ...
    if (/precipitation/.test(variable)) return 'mm';
    if (variable === 'snowfall') return 'cm';
  ...
  function syncBar() {
    ...
    for (const button of state.bar.querySelectorAll('button[data-run-hour]')) {
      ...
      // Een plaats buiten de 39 archiefpunten kan alleen uit de actuele
      // Open-Meteo-run komen. Een andere cyclus aanbieden zou een keuze zijn
      // die bij het tekenen meteen weer wordt teruggedraaid.
      const freePlace = state.locationFallback && hour !== state.selectedHour;
      const available = !!option && !freePlace;
      ...
      const date = option ? new Date(option.timestamp) : null;
      const day = date ? `${date.getUTCDate()} ${['jan','feb','mrt','apr','mei','jun','jul','aug','sep','okt','nov','dec'][date.getUTCMonth()]} · ` : '';
      const label = `${day}${String(hour).padStart(2, '0')} UTC`;
      ...
      button.title = available
        ...
        : freePlace
          ? `${label} bestaat niet voor een plaats buiten de 39 archiefpunten; kies een archiefplaats om deze cyclus weer te kunnen kiezen`
    ...
    if (liveHour == null) {
      ...
      const wanted = state.locationFallbackHour;
      if (state.runChosenByUser && wanted != null && wanted !== state.selectedHour) {
        setStatus(`${String(wanted).padStart(2, '0')} UTC bestaat niet voor deze vrije plaats · ${selected} UTC-run getoond`, true);
      } else {
  ...
  async function initialiseArchive() {
    ...
    try {
      ...
      state.locationFallbackHour = null;
      if (state.liveMeta) {
        state.selectedHour = liveHour();
  ...
  async function ensureLocation(lat, lon) {
    ...
    // De runkeuze van de gebruiker overleeft de plaatswissel: het archief kent
    // alleen de 39 vaste punten, dus een vrije plaats komt uit de actuele
    // Open-Meteo-run. Bewaar het gekozen uur en houd het in de URL, zodat een
    // herlaadbeurt of een stap terug naar een archiefplaats de keuze herstelt
    // in plaats van stilzwijgend op de actuele cyclus te blijven staan.
    if (!state.selectedRun) {
      state.locationFallbackRun = state.locationFallbackRun || state.capabilityFallbackRun;
      state.capabilityFallbackRun = null;
      state.locationFallbackHour = state.locationFallbackHour != null
        ? state.locationFallbackHour : state.selectedHour;
      state.selectedHour = liveHour();
      state.locationFallback = true;
      state.requestedUnavailableHour = null;
      state.reusableMetaResponses = 1;
      rememberFallbackRunInUrl();
      syncBar();
      return { archived: false, fallback: true };
    }
    state.locationFallbackRun = state.selectedRun;
    state.locationFallbackHour = runHour(state.selectedRun) ?? state.selectedHour;
    state.selectedRun = null;
    state.capabilityFallbackRun = null;
    state.selectedHour = liveHour();
    state.locationFallback = true;
  ...
  function rememberFallbackRunInUrl() {
    const hour = state.runChosenByUser && state.locationFallbackHour != null
      ? state.locationFallbackHour : state.selectedHour;
    if (!CYCLES.includes(Number(hour))) return;
    const pageUrl = new URL(location.href);
    pageUrl.searchParams.set('run', String(hour).padStart(2, '0'));
    history.replaceState(null, '', pageUrl);
  }
  ...
  async function ensureVariables(variables) {
    ...
    if (!state.liveMeta) {
      return { archived: true, fallback: false, unavailable: true, missing };
    }
    const selectedRunMs = Date.parse(state.selectedRun.run || '');
    const liveMs = metaRunMs(state.liveMeta);
    if (!Number.isFinite(liveMs) || (Number.isFinite(selectedRunMs) && liveMs < selectedRunMs)) {
      // De vroege ECPDS-run kan vóór de volledige Open-Meteo-run klaarstaan.
      // Een ontbrekend specialistisch veld mag dan niet stilzwijgend uit de
      // oudere livecyclus komen: blijf op de gekozen run en meld de capability.
      return { archived: true, fallback: false, unavailable: true, missing };
    }
  ...
  // Vernieuwen moet ook de gekozen run en het stationarchief opnieuw bepalen.
  // Alleen opnieuw tekenen houdt anders de run van het eerste paginabezoek vast.
  async function refreshSelection() {
    await state.archiveReady;
    const chosenHour = state.runChosenByUser
      ? (state.locationFallbackHour ?? (state.capabilityFallbackRun ? runHour(state.capabilityFallbackRun) : state.selectedHour))
      : null;
    await initialiseMeta();
    archiveCache.clear();
    const archive = await loadArchive('debilt');
    const preferred = chosenHour == null
      ? newestSelectableRun(archive) : selectableRunForHour(archive, chosenHour);
    const option = preferred || newestSelectableRun(archive);
    if (!option) throw new Error('Geen volledige ECMWF-run beschikbaar');
    state.archive = archive;
    state.archiveError = null;
    state.selectedHour = option.hour;
    state.selectedRun = option.run;
    state.requestedUnavailableHour = chosenHour != null && !preferred ? chosenHour : null;
    state.capabilityFallbackRun = null;
    state.locationFallbackRun = null;
    state.locationFallbackHour = null;
    state.locationFallback = false;
  ...
  window.WeerlabPlumeRuns = {
    ...
    assertRunResponse, completeMemberMatrix, refreshSelection, fetchFreshMeta: fetchLiveMeta,
  };
})();
