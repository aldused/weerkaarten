"""
pluim_barendrecht.py — Ensemble pluim Barendrecht
3 panelen: temperatuur / neerslag / wind+stoten
Rode stippellijn bij 10°C grens
Lokaal gebruik
"""
...
print("Data ophalen Barendrecht...")
...
def leden(prefix):
    ...
    for key, vals in hourly.items():
        if key.startswith(prefix + "_member") or key == prefix:
            arr.append(np.array(vals, dtype=float))
...
alle_rr    = np.where(np.isnan(alle_rr), 0, alle_rr)
...
gs  = gridspec.GridSpec(3, 1, figure=fig, height_ratios=[1, 0.6, 0.8], hspace=0.12)
...
header_ax.text(0.012,0.22,f"ECMWF ENS · {alle_temp.shape[0]} leden · {runtime}",
...
ax1.plot(x, med_temp, color="#cc2200", linewidth=2.5, zorder=6, label="Mediaan")
ax1.plot(x, hres_lijn, color="#003366", linewidth=2.0, zorder=7, label="HRES")

# Rode stippellijn bij 10°C
ax1.axhline(10, color="#cc2200", linewidth=1.5, linestyle="--", zorder=8, alpha=0.8)
ax1.text(x[-1], 10.2, "10°C", fontsize=8, color="#cc2200", ha="right", va="bottom", zorder=9)
...
leg1 = [
    Line2D([0],[0], color="#27ae60", lw=1, alpha=0.6, label=f"ENS leden ({alle_temp.shape[0]})"),
    Line2D([0],[0], color="#cc2200", lw=2.5, label="Mediaan"),
    Line2D([0],[0], color="#003366", lw=2.0, label="HRES"),
    Line2D([0],[0], color="#cc2200", lw=1.5, linestyle="--", label="10°C grens"),
]
...
ax2r.set_yticks([0,20,40,60,80,100])
...
ax3.fill_between(x, med_wind, p75_stoot, color="#003366", alpha=0.12, zorder=2)
...
# Beaufort referentielijnen
for bft, ms, lbl in [(3,19,"Bft 3"),(4,29,"Bft 4"),(5,39,"Bft 5"),(6,50,"Bft 6")]:
    ax3.axhline(ms, color="#888888", linewidth=0.6, linestyle=":", zorder=1)
    ax3.text(1, ms+0.5, lbl, fontsize=6.5, color="#888888", ha="left", va="bottom", zorder=9)

for tp in tick_pos:
    ax3.axvline(tp, color="#dddddd", linewidth=0.7, zorder=1)
ax3.set_xticks(tick_pos)
ax3.set_xticklabels(tick_lbl, fontsize=8.5, color="#444444")
