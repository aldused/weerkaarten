...
def zon_elevatie(lat, lon, t_utc):
    """Zonshoogte in graden (declinatie + tijdsvereffening, ~0.5° nauwkeurig)."""
    d = t_utc.timetuple().tm_yday
    decl = math.radians(-23.44 * math.cos(math.radians(360.0/365.25 * (d + 10))))
    B = math.radians(360.0/364.0 * (d - 81))
    eot_min = 9.87*math.sin(2*B) - 7.53*math.cos(B) - 1.5*math.sin(B)
    uur_zon = t_utc.hour + t_utc.minute/60.0 + lon/15.0 + eot_min/60.0
    ha = math.radians(15.0 * (uur_zon - 12.0))
...
# ── Data ophalen ───────────────────────────────────────────────────────────────
def haal_obs(wigos_id, lat, lon):
    """Haal meest recente ta, ff, dd, n op."""
    now_utc = datetime.now(timezone.utc)
    # 60 min venster: stabiele qg-mediaan + fallback als n even ontbreekt
    ...
    try:
        ...
        # De ceilometer-n telt sluierbewolking en passerende brokken vol mee;
        # vergelijk daarom qg met de clear-sky-straling voor de actuele
        # zonshoogte (Holtslag-Van Ulden: 990*sin(h)-30). Mediaan over het
        # venster dempt wolkrand-spikes (ratio kan >1 zijn door reflectie).
        qg_reeks = [to_float(v) for v in (ranges.get("qg", {}).get("values") or [])]
        ratios = []
        for t_str, qg_v in zip(tijden, qg_reeks):
            if qg_v is None: continue
            try:
                t_obs = datetime.fromisoformat(t_str.replace("Z", "+00:00"))
            except ValueError:
                continue
            # qg = gemiddelde over voorgaande 10 min → zonshoogte op t-5min
            el = zon_elevatie(lat, lon, t_obs - timedelta(minutes=5))
            clear = 990.0 * math.sin(math.radians(max(el, 0.0))) - 30.0
            if el < 12.0 or clear < 60.0:
                continue  # zon te laag: ratio onbetrouwbaar
            ratios.append(min(qg_v / clear, 1.5))
        if n is not None and ratios:
            ratios.sort()
            rmed = ratios[len(ratios)//2]
            if   rmed >= 0.85: n = min(n, 1)   # vrijwel volle zon → max 1 okta
            elif rmed >= 0.65: n = min(n, 2)   # overwegend zonnig → max 2 oktas
            elif rmed >= 0.45: n = min(n, 4)   # half bewolkt → max 4 oktas
            elif rmed >= 0.30: n = min(n, 6)   # veel bewolking, af en toe zon
