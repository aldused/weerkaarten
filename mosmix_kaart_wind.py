...
for code, name in stations:
    ...
    for i, dt in enumerate(times):
        ...
        hour = loc.hour
        if 6 <= hour < 18:
            if d not in daily: daily[d] = {"ff":[],"fx":[],"dd":[]}
            if i < len(ff_raw) and ff_raw[i]: daily[d]["ff"].append(ff_raw[i])
            if i < len(fx_raw) and fx_raw[i]: daily[d]["fx"].append(fx_raw[i])
            if i < len(dd_raw) and dd_raw[i]: daily[d]["dd"].append(dd_raw[i])
    days = sorted(daily.keys())[:10]
    for d in days:
        if d not in data_per_day: data_per_day[d] = {}
        ff_gem = sum(daily[d]["ff"])/len(daily[d]["ff"]) if daily[d]["ff"] else 0
        fx_max = max(daily[d]["fx"]) if daily[d]["fx"] else 0
        dd_gem = sum(daily[d]["dd"])/len(daily[d]["dd"]) if daily[d]["dd"] else None
        data_per_day[d][name] = {"ff":ff_gem,"fx":fx_max,"dd":dd_gem}
