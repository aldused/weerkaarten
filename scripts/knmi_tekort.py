#!/usr/bin/env python3
"""
knmi_tekort.py — Neerslagtekort P13 (landelijk + per station) en Rotterdam.
...
ROTTERDAM_AWS = 344
ROTTERDAM_COORDS = (4.447, 51.962)
ROTTERDAM_ACTUEEL = os.path.join(SCRIPT_DIR, "maanddata_344.json")

# EDR-dagwaarnemingen: ~2 dagen verser dan daggegevens.knmi.nl
EDR_DAG = ("https://api.dataplatform.knmi.nl/edr/v1/collections/"
           "daily-in-situ-meteorological-observations")
EDR_FILL_DAGEN = 30   # zoveel dagen terugkijken bij het aanvullen
...
def dagindex(datum, start_maand: int, start_dag: int) -> int | None:
    """Vergelijkbare dagindex; 29 februari wordt overgeslagen in jaarrondreeksen."""
    ts = pd.Timestamp(datum)
    if start_maand == 1 and start_dag == 1:
        if ts.month == 2 and ts.day == 29:
            return None
        return (date(2001, ts.month, ts.day) - date(2001, 1, 1)).days
    return (ts.date() - date(ts.year, start_maand, start_dag)).days
...
def laad_edr_dag(aws: int, na_datum: pd.Timestamp | None) -> pd.DataFrame:
    """
    Recente RH + EV24 uit de EDR-dagwaarnemingen van het KNMI.

    daggegevens.knmi.nl loopt ~2 dagen achter; deze collectie is 's ochtends
    al bij t/m gisteren. Let op de labelconventie: het tijdlabel is de dag ná
    de waarnemingsdag (parameter-duration -P1D), dus label 11 aug = etmaal
    10 aug. Zie ook reference: EDR daily-label = ZIP-label + 1 dag.
    """
    start = pd.Timestamp(datetime.now().date()) - pd.Timedelta(days=EDR_FILL_DAGEN)
    if na_datum is not None:
        start = max(start, na_datum - pd.Timedelta(days=1))
    eind = pd.Timestamp(datetime.now().date()) + pd.Timedelta(days=1)
    try:
        r = knmi_get(
            f"{EDR_DAG}/locations/0-20000-0-06{aws:03d}",
            params={
                "parameter-name": "RH,EV24",
                "datetime": f"{start:%Y-%m-%d}T00:00:00Z/{eind:%Y-%m-%d}T00:00:00Z",
            },
            timeout=60,
        )
        cov = r.json()["coverages"][0]
        tijden = cov["domain"]["axes"]["t"]["values"]
        rng = cov["ranges"]
    except Exception as e:
        print(f"    [warn] EDR-dagdata AWS {aws} mislukt: {e}")
        return pd.DataFrame(columns=["rh", "ev"])
    ...
    for i, ts in enumerate(tijden):
        ...
        datum = pd.Timestamp(ts[:10]) - pd.Timedelta(days=1)
        if na_datum is not None and datum <= na_datum:
            continue
        rows.append((datum, max(float(rh), 0.0), max(float(ev), 0.0)))
    if not rows:
        return pd.DataFrame(columns=["rh", "ev"])
    return pd.DataFrame(rows, columns=["datum", "rh", "ev"]).set_index("datum").sort_index()


_AWS_CACHE: dict[int, pd.DataFrame] = {}

def laad_aws_actueel(aws: int) -> pd.DataFrame:
    """laad_aws() aangevuld met de verse EDR-dagen (gememoïseerd per station)."""
    if aws in _AWS_CACHE:
        return _AWS_CACHE[aws]
    df = laad_aws(aws)
    laatste = df.dropna(subset=["rh", "ev"]).index.max() if not df.empty else None
    extra = laad_edr_dag(aws, laatste)
    if not extra.empty:
        df = pd.concat([df, extra]).sort_index()
        df = df[~df.index.duplicated(keep="last")]
        print(f"    EDR-aanvulling AWS {aws}: +{len(extra)} dagen "
...
def makkink_ev24(tg: float, q: float) -> float:
    """
    KNMI-Makkinkreferentieverdamping in mm/dag uit etmaaltemperatuur (°C)
    en globale straling Q (J/cm²). Formule uit de KNMI-documentatie.
    """
    gamma = 0.646 + 0.0006 * tg
    verhouding = 1.0 + tg / 237.3
    helling = (
        7.5 * math.log(10) * 6.107
        * 10 ** (7.5 * (1.0 - 1.0 / verhouding))
        / (237.3 * verhouding * verhouding)
    )
    return (
        6.5
        * (1.0 - gamma / (helling + gamma))
        / (2501.0 - 2.38 * tg)
        * q
    )


def laad_rotterdam_actueel(na_datum: pd.Timestamp) -> pd.DataFrame:
    """
    Recente KNMI-EDR-dagen uit maanddata_344.json.

    RH is gemeten neerslag. EV24 wordt volgens de KNMI-Makkinkformule
    afgeleid uit de gemeten etmaaltemperatuur TG en globale straling Q.
    """
    if not os.path.exists(ROTTERDAM_ACTUEEL):
        return pd.DataFrame(columns=["rh", "ev"])
    with open(ROTTERDAM_ACTUEEL, encoding="utf-8") as f:
        bron = json.load(f).get("data", {})
    rows = []
    for datum_str, dag in bron.items():
        try:
            ...
            tg = dag.get("tg")
            q = dag.get("q")
            if rh is None or tg is None or q is None:
                continue
            rows.append((datum, max(float(rh), 0.0), round(makkink_ev24(float(tg), float(q)), 1)))
...
def per_jaar_op_doy(tekort_df: pd.DataFrame, jaar_start_maand: int, jaar_start_dag: int) -> dict:
    ...
    for jaar, grp in tekort_df.groupby("jaar"):
        waarden = {}
        for datum, waarde in zip(grp.index, grp["tekort"]):
            doy = dagindex(datum, jaar_start_maand, jaar_start_dag)
            if doy is not None and doy >= 0:
                waarden[int(doy)] = round(float(waarde), 1)
        out[int(jaar)] = waarden
    return out
...
def bereken_ensemble_waaier(forecast_ensemble: dict, actieve_stations: list,
                             ...
                             start_maand: int, start_dag: int,
                             waarnemingen_tm: pd.Timestamp | None) -> list:
    """
    ...
    alle_dates = [pd.Timestamp(d["date"]) for d in forecast_ensemble[eerste_aws]]
    bron_indices = [
        i for i, datum in enumerate(alle_dates)
        if waarnemingen_tm is None or datum > waarnemingen_tm
    ]
    if not bron_indices:
        return []
    dates = [alle_dates[i] for i in bron_indices]
    n_dagen = len(dates)
    ...
    n_members = min(
        len(forecast_ensemble[aws][bron_indices[0]]["precip"])
    ...
    for aws in aws_lijst:
        ...
        for dag_idx, bron_idx in enumerate(bron_indices):
            if bron_idx >= len(ens):
                continue
            ppreds = ens[bron_idx]["precip"][:n_members]
            eepreds = ens[bron_idx]["et0"][:n_members]
    ...
    # Stop bij de eerste dag zonder complete ensemble-informatie; een vlakke
    # lijn door ontbrekende waarden zou ten onrechte als verwachting ogen.
    geldige_dagen = np.all(counts > 0, axis=0)
    eerste_ongeldig = next((i for i, ok in enumerate(geldige_dagen) if not ok), n_dagen)
    if eerste_ongeldig == 0:
        return []
    dates = dates[:eerste_ongeldig]
    precip_mat = precip_mat[:, :eerste_ongeldig]
    et0_mat = et0_mat[:, :eerste_ongeldig]
    counts = counts[:, :eerste_ongeldig]
    n_dagen = eerste_ongeldig
    ...
    for dag_idx in range(n_dagen):
        datum = dates[dag_idx]
        doy = dagindex(datum, start_maand, start_dag)
...
def statistieken_per_doy(per_jaar: dict, vandaag_doy: int, excl_jaar: int | None = None) -> list:
    ...
    for doy in range(0, max_doy + 1):
        ...
        result.append({
            ...
            "p5":     round(float(np.percentile(arr, 5)), 1),   # grens 5% natste jaren
            "p95":    round(float(np.percentile(arr, 95)), 1),  # grens 5% droogste jaren
...
def main():
    ...
    # Laatste dag waarop voor alle actieve stations zowel neerslag als EV24
    # beschikbaar is. Dit is de peildatum voor KPI's en historische ranking.
    observatie_cutoffs = []
    for stn in actieve_stations:
        if rh_per_stn[stn].empty or ev_per_stn[stn].empty:
            continue
        observatie_cutoffs.append(min(rh_per_stn[stn].index[-1], ev_per_stn[stn].index[-1]))
    global_observation_cutoff = min(observatie_cutoffs, default=None)
    ...
    if os.path.exists(FORECAST):
        ...
        for aws_str, info in fcdata.get("stations", {}).items():
            aws = int(aws_str)
            rows = [
                (pd.Timestamp(d["date"]), float(d["precip"]), float(d["et0"] * FAO_TO_MAKKINK))
                for d in info["daily"]
                if d.get("precip") is not None and d.get("et0") is not None
            ]
            if not rows:
                continue
    ...
    # Vul vanaf de eerste ontbrekende waarnemingsdag aan met de forecast.
    for stn in actieve_stations:
        aws = P13_TO_AWS[stn]
        ...
        observatie_tm_stn = min(rh_bestaand.index[-1], ev_per_stn[stn].index[-1])
        fc_add = fc[fc.index > observatie_tm_stn]
        if fc_add.empty:
            continue
        ...
        if not fc_toevoegen.empty:
            ...
            if forecast_end_date is None or fc_toevoegen.index[-1] > forecast_end_date:
                forecast_end_date = fc_toevoegen.index[-1]
    ...
    vandaag_doy_jan = dagindex(datetime.now().date(), 1, 1)
    vandaag_doy_apr = dagindex(datetime.now().date(), 4, 1)
    ...
    peildatum = min(pd.Timestamp(datetime.now().date()), global_observation_cutoff) \
        if global_observation_cutoff is not None else pd.Timestamp(datetime.now().date())
    peil_doy_jan = peil_doy(huidig_jaar_data_jan, dagindex(peildatum, 1, 1))
    peil_doy_apr = peil_doy(huidig_jaar_data_apr, dagindex(peildatum, 4, 1))
    ...
    def bepaal_categorie(huidig_v: float, per_jaar: dict, peil_doy: int, huidig: int) -> dict:
        if huidig_v is None:
            return {"label": "onbekend", "kleur": "#94a3b8", "percentiel": None}
        historisch = [d[peil_doy] for jr, d in per_jaar.items() if jr != huidig and peil_doy in d]
        if not historisch:
            return {"label": "onbekend", "kleur": "#94a3b8", "percentiel": None}
        pct = 100.0 * sum(v <= huidig_v for v in historisch) / len(historisch)
    ...
    categorie_jan = bepaal_categorie(
        huidig_jaar_data_jan.get(peil_doy_jan), land_per_jan, peil_doy_jan, huidig_jaar
    )
    categorie_apr = bepaal_categorie(
        huidig_jaar_data_apr.get(peil_doy_apr), land_per_apr, peil_doy_apr, huidig_jaar
    ...
    ensemble_jan = bereken_ensemble_waaier(
        ...
        huidig_jaar_data_jan, peil_doy_jan, huidig_jaar, 1, 1, global_observation_cutoff
    )
    ensemble_apr = bereken_ensemble_waaier(
        forecast_ensemble, actieve_stations, P13_TO_AWS,
        huidig_jaar_data_apr, peil_doy_apr, huidig_jaar, 4, 1, global_observation_cutoff
    )

    def bouw_lokale_reeks(aws: int, naam: str, lon: float, lat: float) -> dict:
        """Bouw dezelfde monitorvelden voor één zelfstandig KNMI-AWS-station."""
        aws_df = laad_aws(aws)
        rh_obs = aws_df["rh"].dropna()
        ev_obs = aws_df["ev"].dropna()
        if rh_obs.empty or ev_obs.empty:
            raise RuntimeError(f"Geen bruikbare lokale droogtedata voor AWS {aws}")

        observatie_tm = min(rh_obs.index[-1], ev_obs.index[-1])
        rh = rh_obs.copy()
        ev = ev_obs.copy()
        voorlopig_vanaf = None
        voorlopig_tm = None
        lokaal_fc_vanaf = None
        lokaal_fc_tm = None

        # De gevalideerde KNMI-dagreeks loopt enkele dagen achter. Vul de
        # tussenliggende dagen aan met recente EDR-waarnemingen. EV24 wordt
        # daarbij uit gemeten TG en Q berekend met de KNMI-Makkinkformule.
        recent = laad_rotterdam_actueel(observatie_tm)
        if not recent.empty:
            rh = pd.concat([rh, recent["rh"].rename(None)]).sort_index()
            ev = pd.concat([ev, recent["ev"].rename(None)]).sort_index()
            voorlopig_vanaf = recent.index[0]
            voorlopig_tm = recent.index[-1]

        lokale_data_tm = min(rh.index[-1], ev.index[-1])
        if aws in forecast_aws:
            fc = forecast_aws[aws]
            fc_add = fc[fc.index > lokale_data_tm]
            if not fc_add.empty:
                rh = pd.concat([rh, fc_add["rh_fc"].rename(None)]).sort_index()
                ev = pd.concat([ev, fc_add["ev_fc"].rename(None)]).sort_index()
                lokaal_fc_vanaf = fc_add.index[0]
                lokaal_fc_tm = fc_add.index[-1]

        ev_klim = bereken_ev24_klimatologie(ev_obs)
        tek_jan = tekort_per_jaar(rh, ev, 1, 1, ev_klim)
        tek_apr = tekort_per_jaar(rh, ev, 4, 1, ev_klim)
        per_jan = per_jaar_op_doy(tek_jan, 1, 1)
        per_apr = per_jaar_op_doy(tek_apr, 4, 1)

        peildatum_lokaal = min(pd.Timestamp(vandaag), lokale_data_tm)
        peil_jan = peil_doy(per_jan.get(huidig_jaar, {}), dagindex(peildatum_lokaal, 1, 1))
        peil_apr = peil_doy(per_apr.get(huidig_jaar, {}), dagindex(peildatum_lokaal, 4, 1))
        ranking_lokaal_jan = ranking_op_doy(per_jan, peil_jan, huidig_jaar)
        ranking_lokaal_apr = ranking_op_doy(per_apr, peil_apr, huidig_jaar) if peil_apr >= 0 else []

        huidig_jan = per_jan.get(huidig_jaar, {})
        huidig_apr = per_apr.get(huidig_jaar, {})
        proxy_jaren = sorted({
            int(jr) for jr, grp in tek_jan.groupby("jaar") if grp["proxy"].any()
        })
        ...
        return {
            "meta": {
                ...
                "waarnemingen_tm": peildatum_lokaal.strftime("%Y-%m-%d"),
                "definitief_tm": observatie_tm.strftime("%Y-%m-%d"),
                "voorlopig_tm": (
                    voorlopig_tm.strftime("%Y-%m-%d") if voorlopig_tm is not None else None
                ),
                "vandaag_doy_jan": vandaag_doy_jan,
                "vandaag_doy_apr": vandaag_doy_apr,
                "peil_doy_jan": peil_jan,
                "peil_doy_apr": peil_apr,
                "voorlopig_vanaf_doy_jan": (
                    dagindex(voorlopig_vanaf, 1, 1) if voorlopig_vanaf is not None else None
                ),
                "voorlopig_vanaf_doy_apr": (
                    dagindex(voorlopig_vanaf, 4, 1) if voorlopig_vanaf is not None else None
                ),
                "forecast_vanaf_doy_jan": (
                    dagindex(lokaal_fc_vanaf, 1, 1) if lokaal_fc_vanaf is not None else None
                ),
                "forecast_vanaf_doy_apr": (
                    dagindex(lokaal_fc_vanaf, 4, 1) if lokaal_fc_vanaf is not None else None
                ),
                "forecast_vanaf": (
                    lokaal_fc_vanaf.strftime("%Y-%m-%d") if lokaal_fc_vanaf is not None else None
                ),
                "forecast_tm": (
                    lokaal_fc_tm.strftime("%Y-%m-%d") if lokaal_fc_tm is not None else None
                ),
                "periode_jaren": [int(min(per_jan.keys())), int(max(per_jan.keys()))],
                "proxy_jaren": proxy_jaren,
            },
            "per_jaar_jan": per_jan,
            "per_jaar_apr": per_apr,
            "klimaat_jan": statistieken_per_doy(per_jan, vandaag_doy_jan, huidig_jaar),
            "klimaat_apr": statistieken_per_doy(
                per_apr, vandaag_doy_apr if vandaag_doy_apr >= 0 else 0, huidig_jaar
            ),
            "ranking_jan": ranking_lokaal_jan,
            "ranking_apr": ranking_lokaal_apr,
            "ensemble_jan": bereken_ensemble_waaier(
                forecast_ensemble, ["rotterdam"], {"rotterdam": aws},
                huidig_jan, peil_jan, huidig_jaar, 1, 1, lokale_data_tm
            ),
            "ensemble_apr": bereken_ensemble_waaier(
                forecast_ensemble, ["rotterdam"], {"rotterdam": aws},
                huidig_apr, peil_apr, huidig_jaar, 4, 1, lokale_data_tm
            ),
            "categorie_jan": bepaal_categorie(
                huidig_jan.get(peil_jan), per_jan, peil_jan, huidig_jaar
            ),
            "categorie_apr": (
                bepaal_categorie(huidig_apr.get(peil_apr), per_apr, peil_apr, huidig_jaar)
                if peil_apr >= 0 else
                {"label": "voor 1 apr", "kleur": "#94a3b8", "percentiel": None}
            ),
        }

    rotterdam = bouw_lokale_reeks(
        ROTTERDAM_AWS, "Rotterdam Airport", ROTTERDAM_COORDS[0], ROTTERDAM_COORDS[1]
    )
    ...
    output = {
        "meta": {
            ...
            "waarnemingen_tm": peildatum.strftime("%Y-%m-%d"),
            "p13_tm": global_p13_cutoff.strftime("%Y-%m-%d") if global_p13_cutoff is not None else None,
            "vandaag_doy_jan": vandaag_doy_jan,
            "vandaag_doy_apr": vandaag_doy_apr,
            "peil_doy_jan":    peil_doy_jan,
            "peil_doy_apr":    peil_doy_apr,
            "voorlopig_vanaf_doy_jan": (
                dagindex(global_p13_cutoff + pd.Timedelta(days=1), 1, 1)
                if global_p13_cutoff is not None and global_p13_cutoff.year == huidig_jaar else None
            ),
            "voorlopig_vanaf_doy_apr": (
                dagindex(global_p13_cutoff + pd.Timedelta(days=1), 4, 1)
                if global_p13_cutoff is not None and global_p13_cutoff.year == huidig_jaar else None
            ),
            "forecast_vanaf_doy_jan": (
                dagindex(forecast_start_date, 1, 1)
                if forecast_start_date is not None else None
            ),
            "forecast_vanaf_doy_apr": (
                dagindex(forecast_start_date, 4, 1)
                if forecast_start_date is not None else None
            ),
            "forecast_vanaf": forecast_start_date.strftime("%Y-%m-%d") if forecast_start_date is not None else None,
            "forecast_tm": forecast_end_date.strftime("%Y-%m-%d") if forecast_end_date is not None else None,
    ...
    print(f"\n── Huidig jaar {huidig_jaar} op waarnemingspeildatum {peildatum.date()} ──")
    if peil_doy_jan in huidig_jaar_data_jan:
        print(f"  Landelijk tekort (sinds 1 jan): {huidig_jaar_data_jan[peil_doy_jan]:.1f} mm")
    if peil_doy_apr >= 0 and peil_doy_apr in huidig_jaar_data_apr:
        print(f"  Landelijk tekort (sinds 1 apr): {huidig_jaar_data_apr[peil_doy_apr]:.1f} mm")
