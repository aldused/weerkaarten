    ...
    for regel in tekst.splitlines():
        ...
        def get(naam):
            try:
                ...
                v = delen[idx].strip()
                # Alleen lege etmgeg-cellen zijn ontbrekend. Een letterlijke 9
                # betekent voor TX/TN/TG/RH/SQ de geldige meetwaarde 0,9.
                if v == "":
                    return None
                f = float(v)
                if naam in ("RH", "SQ"):
...
def bereken_records(dag: pd.Series, param: str) -> dict:
    ...
    def _decade_rows(sub, ascending):
        s = sub.sort_values("val", ascending=ascending).head(TOP_N)
        return [{"label": f"{decade_label(int(r.mnd), (int(r.dec)-1)*10+1)} {int(r.jaar)}",
                 "waarde": round(float(r.val), 1)} for r in s.itertuples(index=False)]

    per_mnd_dec_hoog = {}
    per_mnd_dec_laag = {}
    for m in range(1, 13):
        mdf = grp[grp["mnd"] == m]
        hoog = {"all": _decade_rows(mdf, False)}
        laag = {"all": _decade_rows(mdf, True)}
        for d in (1, 2, 3):
            ddf = mdf[mdf["dec"] == d]
            hoog[str(d)] = _decade_rows(ddf, False)
            laag[str(d)] = _decade_rows(ddf, True)
        per_mnd_dec_hoog[str(m)] = hoog
        if is_temp or is_zon:
            per_mnd_dec_laag[str(m)] = laag
    records["hoogste_decade_per"] = per_mnd_dec_hoog
    if is_temp or is_zon:
