...
print("MOSMIX ophalen (min temp nacht 18-06u België)...")
data_per_day = {}
...
for code, naam in stations:
    ...
    days = [d for d in sorted(daily_tn.keys()) if d >= vandaag][:10]
    for d in days:
        if d not in data_per_day: data_per_day[d] = {}
        data_per_day[d][naam] = round(daily_tn[d], 1)
...
for day, dag_data in data_per_day.items():
    ...
    ax_h.text(0.988, 0.20, f"DWD MOSMIX  ·  run: {now_str}",
              fontsize=8, color="#94a3b8", ha="right", va="center",
    ...
    for naam, tn_v in dag_data.items():
        if naam not in coords: continue
        lon, lat = coords[naam]
        ax.text(lon, lat + 0.03, f"{tn_v:.1f}",
    ...
    leg = ax.inset_axes([0.01, 0.02, 0.16, 0.09])
    ...
    leg.add_patch(plt.Rectangle((0.05,0.18),0.16,0.64,facecolor="#1a5fb4",
                  transform=leg.transAxes,zorder=1))
    leg.text(0.27, 0.50, "Min temp nacht (18–06u)", fontsize=5, va="center", transform=leg.transAxes)

    ax.text(1.0, 0.0, f"© Ed Aldus | Data: DWD (MOSMIX) | {now_str2}",
            transform=ax.transAxes, fontsize=7, style="italic",
            ha="right", va="bottom", color="#555555")

    fname = f"kaart_be_temp_nacht_{dag_nl.lower()}_{day.strftime('%d%b%Y').lower()}.png"
...
for oud in sorted(_glob.glob("kaart_be_temp_nacht_*.png"), key=os.path.getmtime)[:-10]:
