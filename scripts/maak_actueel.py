...
Haalt alle synop-velden en het dagverloop vanaf lokale middernacht op en
slaat dit op als actueel.json.
"""
...
HITTEKRACHT_JSON = "hittekracht_gemeten.json"
HITTEKRACHT_MAX_OUDERDOM = timedelta(minutes=45)
...
STATIONS = {
    ...
    "0-20000-0-06391": ("Arcen",           6.196, 51.500),
    "0-528-0-06392":   ("Horst",           6.029, 51.449),
}

def laad_hittekracht():
    """Lees alleen verse officiële WBGT/hittekrachtwaarden van KNMI in."""
    try:
        with open(HITTEKRACHT_JSON) as f:
            data = json.load(f)
        laatste = datetime.fromisoformat(data["laatste_tijd"].replace("Z", "+00:00"))
        ouderdom = datetime.now(timezone.utc) - laatste
        if ouderdom < timedelta(minutes=-5) or ouderdom > HITTEKRACHT_MAX_OUDERDOM:
            print(f"  WBGT/hittekracht overgeslagen: bron is {ouderdom.total_seconds()/60:.0f} min oud")
            return {}, None
        return data.get("stations", {}), data.get("laatste_tijd")
    except (OSError, ValueError, KeyError, TypeError) as e:
        print(f"  WBGT/hittekracht niet beschikbaar: {e}")
        return {}, None

def haal_station(wigos, naam, lon, lat):
    nu  = datetime.now(timezone.utc)
    # Vanaf 00:00 Nederlandse tijd ophalen. Zo kan Opers-Multi bij een eerste
    # bezoek meteen het volledige gemeten temperatuurverloop van vandaag tonen.
    nu_lokaal = nu.astimezone(LOCAL_TZ)
    van = nu_lokaal.replace(hour=0, minute=0, second=0, microsecond=0).astimezone(timezone.utc)
    ...
    try:
        ...
        # Dag-tot-nu-historie van ta/td/ff/dd/fx — voor grafieken/OBS-overlay
...
# Publiceer bij een tijdelijke KNMI/API-storing nooit een leeg of sterk
# onvolledig bestand over de laatste geldige waarnemingen heen.
if len(resultaten) < 20:
    raise SystemExit(
        f"AFGEBROKEN: slechts {len(resultaten)} stations opgehaald; "
        "bestaande actueel.json blijft behouden"
    )

# WBGT en hittekracht komen uit de officiële KNMI-dataset
# wet_bulb_globe_temperature 3.0. Deze bevat alleen deelnemende stations.
hitte_stations, hitte_tijd = laad_hittekracht()
for wigos, (naam, _lon, _lat) in STATIONS.items():
    station = resultaten.get(naam)
    hitte = hitte_stations.get(wigos.rsplit("-", 1)[-1])
    if not station or not hitte:
        continue
    wbgt = hitte.get("wbgt_nu")
    hk = hitte.get("hk_nu")
    if wbgt is not None:
        station["wbgt"] = round(float(wbgt), 1)
    if hk is not None:
        station["hk"] = int(hk)

nu_str = datetime.now(LOCAL_TZ).strftime("%d %b %Y %H:%M")
uitvoer = {"bijgewerkt": nu_str, "stations": resultaten}
if hitte_tijd:
    uitvoer["wbgt_bijgewerkt"] = hitte_tijd
