...
_cli.add_argument(
    ...
    action="append",
    dest="stations",
    help="genereer alleen dit stationsnummer (optie mag vaker)",
)
SELECTED_STATIONS = set(_cli.parse_args().stations or [])
...
def download_knmi(station, cache_path):
    ...
    # Bewaar de bestaande cache tot de download én ZIP-decodering zijn gelukt.
    # Zo blijft een bruikbaar KNMI-bestand staan bij een tijdelijke netwerkfout.
...
def parse_csv(tekst):
    ...
    for regel in tekst.splitlines():
        ...
        def val(naam):
            try:
                ...
                v = delen[idx].strip()
                # In etmgeg-bestanden is uitsluitend een lege cel ontbrekend.
                # De waarde 9 is voor deze velden een geldige 0,9 (bijv. °C,
                # mm of uur). Alleen NG gebruikt 9 als bijzondere code, maar
                # bewolking wordt hier niet ingelezen.
                if v == "": return None
                f = float(v)
    ...
    # KNMI-bestanden kunnen incidenteel overlappende/verdubbelde datumregels
    # bevatten. Combineer die tot één etmaal; een latere niet-lege waarde geldt
    # als de meest recente broncorrectie.
    per_datum = {}
    for record in records:
        bestaand = per_datum.get(record["datum"])
        if bestaand is None:
            per_datum[record["datum"]] = record
            continue
        for sleutel, waarde in record.items():
            if waarde is not None:
                bestaand[sleutel] = waarde
    return [per_datum[datum] for datum in sorted(per_datum)]
...
# ── Lopende dagen via EDR (sluit het ~2-daagse ZIP-gat) ─────────────────────────
# De etmgeg-ZIP loopt 1–2 dagen achter. Vul aan tot en met vandaag met:
#   • gefinaliseerde dagen → daily-...-validated  (label = dag+1 op 00:00Z)
#   • vandaag (lopend)     → 10-minuten API, geaggregeerd tot etmaalwaarden
from knmi_api import knmi_get
from datetime import timezone

EDR_DAG = "https://api.dataplatform.knmi.nl/edr/v1/collections/daily-in-situ-meteorological-observations-validated"
...
# valt terug op 0-20000-0 als de lijst (nog) niet beschikbaar is.
_WIGOS_CACHE = None
def _wigos(station_nr):
    ...
    if _WIGOS_CACHE is None:
        ...
        try:
            ...
            print(f"  WIGOS-locations ophalen mislukt: {e}")
...
def haal_dag_validated(station_nr, dag):
    """Gefinaliseerde etmaalwaarden voor `dag` uit de validated daily-API.
    Label-conventie: de waarde voor dag D staat op tijdstip (D+1)T00:00Z."""
    label = f"{(dag + timedelta(days=1)).isoformat()}T00:00:00Z"
    par   = "TX,TN,TG,RH,FXX,FG,FHX,PG,PX,PN,SQ"
    try:
        r = knmi_get(f"{EDR_DAG}/locations/{_wigos(station_nr)}",
                     params={"datetime": label, "parameter-name": par}, timeout=20)
        if r.status_code != 200:
            return None
        cov = r.json().get("coverages")
        if not cov:
            return None
        rng = cov[0].get("ranges", {})
        def g(k):
            v = _edr_floats(rng.get(k, {}).get("values"))
            return v[0] if v and v[0] is not None else None
        # EDR-mapping → records-sleutels (FXX→fx, FHX→fhx, overige gelijk)
        w = {"tx": g("TX"), "tn": g("TN"), "tg": g("TG"), "rh": g("RH"),
             "fx": g("FXX"), "fg": g("FG"), "fhx": g("FHX"),
             "pg": g("PG"), "px": g("PX"), "pn": g("PN"), "sq": g("SQ")}
        return w if (w["tx"] is not None or w["tg"] is not None) else None
    except Exception as e:
        print(f"  EDR-dag {station_nr} {dag}: {e}")
        return None
...
def maak_dagrecord(station_nr, datum_obj, waarden, lopend=False):
    ...
    j, m, d = datum_obj.year, datum_obj.month, datum_obj.day
    rec = {
        "datum":   datum_obj.isoformat(),
        "jaar":    j, "maand": m, "dag": d,
        "decade":  min(((d - 1) // 10) + 1, 3),
        "seizoen": seizoen(m),
        "lopend":  lopend,
    }
    for k in ("tx","tn","tg","rh","fx","fg","fhx","pg","px","pn","sq"):
        rec[k] = waarden.get(k)
    return rec

def vul_lopende_dagen(station_nr, data):
    """Voeg de ontbrekende dagen (na het ZIP-gat) t/m vandaag toe.
    Gefinaliseerde dagen via validated daily-API, vandaag lopend via 10-min."""
    if not data:
        return data
    vandaag  = date.today()
    aanwezig = {r["datum"] for r in data}
    extra    = []
    # Begin direct na de laatste ZIP-dag, maar nooit verder dan ~7 dagen terug.
    # Actieve stations hebben hooguit een paar dagen ZIP-gat; een station dat lang
    # geleden is gesloten (reeks eindigt jaren terug) mag NIET dag-voor-dag via de
    # EDR-API worden bijgevuld — dat zijn duizenden zinloze calls (rate-limit).
    d = max(date.fromisoformat(data[-1]["datum"]) + timedelta(days=1),
            vandaag - timedelta(days=7))
    while d <= vandaag:
        if d.isoformat() not in aanwezig:
            # Gefinaliseerde dag → validated daily-API. Die loopt echter ~1 dag
            # achter: 'gisteren' (vandaag-1) staat er vaak nog niet in (HTTP 404).
            # Val dan terug op de 10-min-aggregatie (zoals voor vandaag), zodat de
            # recente nacht/dag toch in de records komt. Self-healing: de volgende
            # ochtendrun pakt alsnog de validated waarde zodra die gepubliceerd is.
            w, is_lopend = None, False
            if d < vandaag:
                w = haal_dag_validated(station_nr, d)
            if w is None:
                w = haal_dag_lopend(station_nr, d)
                is_lopend = True
            if w:
                extra.append(maak_dagrecord(station_nr, d, w, lopend=is_lopend))
        d += timedelta(days=1)
    if extra:
        data.extend(extra)
        data.sort(key=lambda r: r["datum"])
        oms = ", ".join(f"{r['datum']}{' (lopend)' if r.get('lopend') else ''}" for r in extra)
        print(f"  + EDR aangevuld: {oms}")
    return data
...
for STATION, STATION_NAAM in STATIONS:
    ...
    # Sluit het ZIP-gat: vul aan tot en met vandaag (lopend) via EDR
    data = vul_lopende_dagen(STATION, data)
    lopend_datum = next((r["datum"] for r in reversed(data) if r.get("lopend")), None)
