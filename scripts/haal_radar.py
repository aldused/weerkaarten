#!/usr/bin/env python3
"""
haal_radar.py — Haal KNMI radardata op (gauge-gecorrigeerd) + KNMI 2u-nowcast,
exporteer als binair bestand voor de webviewer.

Datasets:
  • nl_rdr_data_rtcor_5m v1.0  → 5-min real-time accumulaties, gauge-gecorrigeerd
  • radar_forecast v2.0        → officiële KNMI radar-nowcast, +0..+120 min in 5-min stappen
...
N_HIST_FRAMES = 36   # 3 uur historie (36 × 5 min)
N_FCST_FRAMES = 24   # 2 uur nowcast (24 × 5 min, slaan +0 over want = nu)
ENABLE_700HPA_BLEND = False  # officiële KNMI-nowcast ongewijzigd tonen
...
# KNMI radar projectie — exact de proj4-string uit de HDF5-attributen
# (projection_proj4_params). a/b staan in km zodat x/y direct in km-grid vallen.
...
#
# Geo-conventies (gecheckt tegen de HDF5-attributen, geo_pixel_def = 'LU'):
#   • x = (col + geo_column_offset) * geo_pixel_size_x   — LU-hoek van pixel
#   • y = (row + geo_row_offset)    * geo_pixel_size_y   — pixel_size_y < 0
#   • pixel-CENTRUM ligt dus op index + 0.5 → dichtstbijzijnde pixel = floor()
#   • pixel size is 1.0000035 / 1.0000048 km, niet exact 1.0
# Target-grid wordt op CELCENTRA gesampled: cel i dekt
# [MIN + i·stap, MIN + (i+1)·stap], centrum op i+0.5. De frontend tekent het
# raster edge-to-edge over precies dit bereik — samplen op celcentra maakt
# dat pixel-exact (voorheen: linspace op celranden + round() ⇒ ~0,5–0,7 km
# systematische NW-verschuiving van het hele radarbeeld).
# ─────────────────────────────────────────────────────────────────────────────
_LUT_CACHE = {}  # attrs-signatuur → (row_idx, col_idx) int-arrays


def _target_celcentra():
    lon_stap = (LON_MAX - LON_MIN) / TARGET_NLON
    lat_stap = (LAT_MAX - LAT_MIN) / TARGET_NLAT
    target_lons = LON_MIN + (np.arange(TARGET_NLON) + 0.5) * lon_stap
    target_lats = LAT_MAX - (np.arange(TARGET_NLAT) + 0.5) * lat_stap  # top-down
    return np.meshgrid(target_lons, target_lats)


def _maak_lookup_tabel(px, py, col_off, row_off, nrows, ncols):
    lon_grid, lat_grid = _target_celcentra()
    ...
    col_f = x_stere / px - col_off
    row_f = y_stere / py - row_off       # py < 0 → row groeit naar het zuiden
    col_idx = np.clip(np.floor(col_f).astype(np.int32), 0, ncols - 1)
    row_idx = np.clip(np.floor(row_f).astype(np.int32), 0, nrows - 1)
    return row_idx, col_idx


def _lookup_voor(h5f):
    """Lookup-tabel op basis van de geo-attributen van dít bestand (gecachet)."""
    g = h5f["geographic"]
    pixel_def = g.attrs.get("geo_pixel_def", b"LU")
    pixel_def = pixel_def.decode() if isinstance(pixel_def, bytes) else str(pixel_def)
    if pixel_def.strip().upper() != "LU":
        print(f"  WAARSCHUWING: onverwachte geo_pixel_def={pixel_def!r}, LU aangenomen")
    sig = (float(g.attrs["geo_pixel_size_x"][0]), float(g.attrs["geo_pixel_size_y"][0]),
           float(g.attrs["geo_column_offset"][0]), float(g.attrs["geo_row_offset"][0]),
           int(g.attrs["geo_number_rows"][0]), int(g.attrs["geo_number_columns"][0]))
    if sig not in _LUT_CACHE:
        _LUT_CACHE[sig] = _maak_lookup_tabel(*sig)
    return _LUT_CACHE[sig]
...
# ─────────────────────────────────────────────────────────────────────────────
def _img_to_mmh(image_data, lut):
    """
...
def parse_h5_history(path):
    ...
    with h5py.File(path, "r") as f:
        return _img_to_mmh(f["image1/image_data"][:], _lookup_voor(f))
...
def main():
    ...
    if os.path.exists(meta_path):
        with open(meta_path) as f:
            old = json.load(f)
        if (
            old.get("nieuwste") == nieuwste_hist
            and old.get("nieuwste_forecast") == nieuwste_fcst
            and old.get("radar_pipeline") == PIPELINE_ID
        ):
            print("  Al up-to-date.")
            return
    ...
    if fcst_files:
        try:
            ...
            nieuwste_hist_time = parse_timestamp(hist_files[-1]["filename"])
            for lead, (fr, ts_str) in enumerate(zip(frames, tijden_str), start=1):
                ts = parse_forecast_valid(ts_str)
                if ts is None:
                    # fallback: extrapolatie vanaf bestandsnaam
                    ts = base + timedelta(minutes=5 * lead)
                if ts <= nieuwste_hist_time:
                    continue
    ...
    # van KNMI/Buienradar, dus die blijft uit tenzij expliciet opnieuw aangezet.
    blend_meta = {
        "active": False,
        "reason": "officiële KNMI radar_forecast ongewijzigd gebruikt",
    }
    if ENABLE_700HPA_BLEND and fcst_frames and len(hist_frames) >= 2:
        try:
        ...
        print("  700 hPa-blend uit: KNMI radar_forecast wordt native gebruikt.")
    ...
    product_naam = "KNMI Regenradar (gauge-gecorrigeerd) + KNMI 2u-nowcast"
    if blend_meta.get("active"):
        product_naam += " + 700 hPa-blend"

    meta = {
        "product": product_naam,
        "radar_pipeline": PIPELINE_ID,
        "forecast_policy": "KNMI radar_forecast v2.0 ongewijzigd tot +2 uur; geen lokale wind-warp.",
        ...
        "grid": {
            ...
            "lon_min": LON_MIN,
            "lon_max": LON_MAX,
            "conventie": "celcentra",  # cel i dekt [min+i·stap, min+(i+1)·stap], waarde = centrum
        },
        "calibratie": {
    ...
    # 9. Upload naar R2
    if "--geen-upload" in sys.argv:
        print("7. Upload overgeslagen (--geen-upload); bestanden staan lokaal klaar.")
        return
