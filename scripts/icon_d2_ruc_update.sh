...
MISSING = 9.9e30  # sentinel voor bitmap-missing (eccodes vult hiermee gemaskeerde punten)
            ...
            with open(tmp.name, "rb") as fh:
                ...
                if msgid:
                    ...
                    if gtype in ("regular_ll", "rotated_ll"):
                        ...
                        # Echte missing-detectie via bitmap-sentinel: een magnitude-clip
                        # (>9000) sloopt legitieme VIS (zicht tot ~100 km) en PMSL (Pa).
                        eccodes.codes_set(msgid, "missingValue", MISSING)
                        ...
                        vals_flat[vals_flat >= MISSING] = np.nan
                        vals_flat = info["conv"](vals_flat)

                        if lats is None:
                            grid_type = "unstructured"
                            # CLAT/CLON = constant grid (verandert nooit per run),
                            # dus 1x downloaden en op schijf cachen. Voorkomt dat
                            # een transiente CLAT/CLON-fout de hele run sloopt.
                            print(f"   Ongestructureerd grid ({n_pts} punten), gridcoördinaten laden...")
                            GRID_CACHE = "icond2ruc_grid_coords.npz"

                            def read_grib_values(data):
                                if not data or data[:4] != b"GRIB":
                                    raise ValueError("geen geldige GRIB ontvangen")
                                with tempfile.NamedTemporaryFile(suffix=".grib2", delete=True) as t:
                                    ...
                                    with open(t.name, "rb") as ff:
                                        ...
                                        if mid is None:
                                            raise ValueError("kon GRIB niet lezen")
                                        v = eccodes.codes_get_values(mid)
                                        eccodes.codes_release(mid)
                                        return v

                            grid_lats_flat = grid_lons_flat = None
                            # 1) Probeer cache
                            if os.path.exists(GRID_CACHE):
                                try:
                                    _c = np.load(GRID_CACHE)
                                    if int(_c["n"]) == int(n_pts):
                                        grid_lats_flat = _c["lat"]
                                        grid_lons_flat = _c["lon"]
                                        print("   Gridcoördinaten uit cache")
                                except Exception as e:
                                    print(f"   Cache onbruikbaar: {e}")
                            # 2) Anders downloaden (met pogingen) en cachen
                            if grid_lats_flat is None:
                                ...
                                for poging in range(1, 4):
                                    try:
                                        ...
                                        print(f"   CLAT/CLON poging {poging} mislukt: {e}")
                                        grid_lats_flat = grid_lons_flat = None
                                        time.sleep(5)
                                if grid_lats_flat is None:
                                    raise RuntimeError("CLAT/CLON konden niet geladen worden")
                                ...
                                try:
                                    np.savez(GRID_CACHE, lat=grid_lats_flat, lon=grid_lons_flat, n=np.array(int(n_pts)))
                                    print("   Gridcoördinaten gecachet")
                                except Exception as e:
                                    print(f"   Cache opslaan mislukt: {e}")
