#!/usr/bin/env python3
"""
ICON-EU via Open-Meteo naar het canvas-formaat van demo_vierluik_neerslag.html.

Gebruik:
    python3 scripts/icon_eu_update.py
"""
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--prefix", default="icon_eu")
    p.add_argument("--model", default="icon_eu")
...
def post_bulk(batch: list[tuple[int, int, float, float]], args: argparse.Namespace) -> list[dict] | None:
    ...
    raise RuntimeError(f"Open-Meteo ICON-EU call mislukt: {last_err}")
...
def main() -> int:
    ...
    print(f"ICON-EU: {n_lat}x{n_lon} = {len(coords)} punten, {len(batches)} batches")
    ...
    meta = {
        "model": f"DWD ICON-EU via Open-Meteo ({args.model})",
