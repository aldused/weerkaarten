...
export async function handleEns6(url, request, env, fetcher=fetch) {
  ...
  try {
    ...
    const manifest = await json(`${ROOT}/pluim_archive_meta.json`,30);
    ...
    if (url.pathname === '/ens6-runs') {
      const runs = (station ? entries : entries.filter(e=>e.run===liveId)).map(e=>({run:e.run, fields:e.fields, revision:e.data_sha256 || manifest.revision}));
      return response({runs, station:station?.slug || null, model:MODEL, revision:manifest.revision});
    }
    const id = runId(url.searchParams.get('run'));
    const entry = entries.find(e=>e.run === id);
    ...
    data.revision = entry.data_sha256 || manifest.revision;
