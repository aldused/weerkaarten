 ...
 *
 * Publieke functie:
 *   WBPluimExport.genereerLossePluimenPNGs({lat, lon, naam, startDate, endDate}) -> Promise
 */
(function(){
  ...
  const PARAMS = {
    ...
    raincum: {
      ...
      makeStats(times, members) {
        return makeDefaultStats(times, cumulativeMembers(members));
      },
      summary(stats) {
        const vals = stats.filter(s => Number.isFinite(s.p50));
    ...
    wind: {
      ...
      rangePad: 1,
      leftAxis: 74,
      statusSuffix: 'windkracht in Beaufort',
      lineLabel: 'Ensemble mediaan',
      bandLabel: 'Onzekerheidsband (P10-P90)',
      memberTransform: kmhToBft,
      scale: [[0,'#4d9b4d'],[2,'#9bc95c'],[4,'#d9c750'],[5,'#ef9a3a'],[7,'#d9573b'],[8,'#9f3251'],[10,'#50245f']],
      summary(stats) {
        const vals = stats.filter(s => Number.isFinite(s.p50));
        ...
        return [
          `Meeste windkracht: ${fmtValue(peak.p50, this)} · ${fmtDag(peak.t)}`,
          `Bovenkant ensemble: ${fmtValue(gusty.p90, this)}`,
          `Meeste onzekerheid: ±${Math.round((spread.p90-spread.p10)/2)} Bft`,
        ];
      },
    },
    gust: {
    ...
    thunder: {
      ...
      render: 'bars',
      barValue: 'p90',
      statusSuffix: 'CAPE P90 + kanssignaal',
      barLabel: 'Staafjes: bovenkant CAPE (P90)',
      percentLine: { key: 'strong', label: 'Kans 500+ J/kg (%)', color: 'white' },
      legendItems: [
      ...
      summary(stats) {
        ...
        const strong = vals.reduce((a,b) => b.extra.strong > a.extra.strong ? b : a, vals[0]);
        return [
          `Hoogste CAPE P90: ${Math.round(peak.p90)} J/kg · ${fmtDag(peak.t)}`,
          `Duiding: ${capeDuiding(peak.p90)}`,
          `Kans 500+ J/kg: ${Math.round(strong.extra.strong)}% · ${fmtDag(strong.t)}`,
        ];
      },
    },
  };
  ...
  function membersOf(hourly, key) {
    return Object.keys(hourly)
      .filter(k => k === key || k.startsWith(`${key}_member`))
      .sort((a, b) => {
        if (a === key) return -1;
        if (b === key) return 1;
  ...
  function aggregateMembersByHours(times, members, hours = 6) {
    ...
    times.forEach((t, i) => {
      ...
      start.setHours(Math.floor(d.getHours() / hours) * hours, 0, 0, 0);
      const key = start.toISOString();
      if (!byKey.has(key)) {
        const group = { key, start, idxs: [] };
        byKey.set(key, group);
        groups.push(group);
      }
      byKey.get(key).idxs.push(i);
    });
    return {
      times: groups.map(group => group.start),
      members: members.map(member => groups.map(group => {
        const vals = group.idxs.map(i => member[i]).filter(Number.isFinite);
  ...
  function cumulativeMembers(members) {
    return members.map(member => {
      ...
      return member.map(value => {
        if (!Number.isFinite(value)) return seen ? sum : null;
  ...
  function drawChart(ctx, x, y, w, h, times, stats, cfg) {
    ...
    const xF = i => x + m.left + (i / Math.max(1, nMax - 1)) * iw;
    ...
    const step = cfg.sampleStep || 3;
    const lineStep = cfg.render === 'bars' ? step : 1;
    const sampledPoints = key => st.map((s, i) => (
      i % lineStep === 0 && Number.isFinite(s[key]) ? [xF(i), yF(s[key])] : null
    )).filter(Boolean);
    const sampledBand = () => st.map((s, i) => (
      i % lineStep === 0 && Number.isFinite(s.p90) && Number.isFinite(s.p10)
    ...
    if (cfg.render === 'stacked') {
      ...
      st.forEach((s, i) => {
        ...
        if (i % step !== 0 || !Number.isFinite(barValue)) return;
      ...
      if (cfg.percentLine) {
        const pctPts = st.map((s, i) => {
          ...
          return i % step === 0 && Number.isFinite(v) ? [xF(i), yPercentF(v)] : null;
        }).filter(Boolean);
        ctx.lineCap = 'round';
        ctx.lineJoin = 'round';
    ...
    for (let v = Math.ceil(yMin / cfg.yStep) * cfg.yStep; v <= yMax; v += cfg.yStep) {
      ...
      if (cfg.rightAxis === 'bft') {
        ctx.textAlign = 'left';
        ctx.fillText(`${kmhToBft(v)} Bft`, x + m.left + iw + 7, yy);
      }
    }
  ...
  async function fetchEnsemble(lat, lon, startIso, endIso) {
    const params = new URLSearchParams({
      latitude: lat,
      longitude: lon,
      hourly: 'temperature_2m,precipitation,windspeed_10m,wind_gusts_10m,cloudcover,cape',
      models: 'ecmwf_ifs025',
      start_date: startIso,
      end_date: endIso,
    ...
    const r = await fetch(rewriteOM(`${ENS_URL}?${params}`));
    if (!r.ok) throw new Error('Ensemble HTTP ' + r.status);
    return r.json();
  }

  function buildModels(ens) {
    const hourly = ens.hourly || {};
    const times = (hourly.time || []).map(t => new Date(t));
    if (!times.length) throw new Error('Geen ensembledata ontvangen');
    return MULTI_PARAMS.map(param => {
      const cfg = PARAMS[param];
      const membersRaw = membersOf(hourly, cfg.hourly);
      const members = cfg.memberTransform
        ? membersRaw.map(member => member.map(cfg.memberTransform))
        : membersRaw;
      if (!members.length) throw new Error(`Geen leden voor ${cfg.title}`);
      const stats = cfg.makeStats ? cfg.makeStats(times, members, hourly) : makeDefaultStats(times, members);
      return { param, cfg, times: chartTimesForStats(times, stats), stats, keys: members.length };
    });
  }
  ...
  function drawPage(ctx, naam, models, resize = true) {
    ...
    ctx.fillText('8-pluim', PAGE_MARGIN, 92);
    const titleW = ctx.measureText('8-pluim').width;
    ...
    models.forEach((model, i) => {
      ...
      ctx.font = '800 23px Inter, Arial, sans-serif';
      ctx.textAlign = 'right';
      ctx.fillText(`${model.keys} leden · ${FORECAST_DAYS} dagen · ${model.cfg.statusSuffix}`, PAGE_W - PAGE_MARGIN, y + 36);
      ctx.textAlign = 'left';
  ...
  function validateInput({lat, lon, naam, startDate, endDate}) {
    ...
    return { naam: naam || 'Plaats', startIso: isoDatum(start), endIso: isoDatum(end) };
  }
  ...
  async function genereerPluimPNG({lat, lon, naam, startDate, endDate, scale=2}) {
    const { naam: naamOut, startIso, endIso } = validateInput({lat, lon, naam, startDate, endDate});
    const ens = await fetchEnsemble(lat, lon, startIso, endIso);
    const models = buildModels(ens);
  ...
  async function genereerLossePluimenPNGs({lat, lon, naam, startDate, endDate, scale=2, zip=false, folderName, params=null}) {
    const { naam: naamOut, startIso, endIso } = validateInput({lat, lon, naam, startDate, endDate});
    const ens = await fetchEnsemble(lat, lon, startIso, endIso);
    const allModels = buildModels(ens);
    const models = params ? allModels.filter(m => params.includes(m.param)) : allModels;
    const results = [];
    ...
    return { files: results.map(({ blob, ...r }) => r), count: results.length };
  }
