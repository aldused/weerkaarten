...
# Onbetrouwbare/foutieve perioden per station (inclusief, YYYYMM).
# Deze rijen worden uitgesloten van recordberekening.
BLACKLIST = {
...
# Handmatige maandcorrecties: station -> [(jaar, mnd, mm)].
# Voor maanden waar de ruwe dagdata corrupt/nul is (en dus geblacklist),
# maar het werkelijke maandtotaal bekend is uit het KNMI-maandoverzicht.
# De correctie wordt ná blacklist-filtering geïnjecteerd in de maandrecords.
MANUAL_MONTH = {
    915: [(1997, 1, 1.0)],  # Eersel: januari 1997 = 1,0 mm (droogste januari ooit; ruwe dagdata = nul-data → geblacklist)
}

def filter_blacklist(nr, data):
    """Verwijder rijen die in de blacklist-perioden van dit station vallen."""
    ranges = BLACKLIST.get(int(nr))
    if not ranges:
        return data
    out = []
    for rij in data:
        ...
        if len(d) < 6:
            ...
            continue
        ym = int(d[:6])
        if any(a <= ym <= b for a, b in ranges):
            continue
        out.append(rij)
    return out
...
def bereken_records(data, nr=None):
    ...
    # Handmatige maandcorrecties injecteren (bv. Eersel jan 1997: ruwe dagdata corrupt/nul)
    for mjaar, mmnd, mmm in MANUAL_MONTH.get(int(nr), []) if nr is not None else []:
        entry = {"waarde": round(mmm, 1), "label": f"{NL_MND_LANG[mmnd]} {mjaar}",
                 "jaar": mjaar, "mnd": mmnd}
        mnd_alle[:]       = [r for r in mnd_alle       if not (r["jaar"] == mjaar and r["mnd"] == mmnd)]
        droog_mnd_alle[:] = [r for r in droog_mnd_alle if not (r["jaar"] == mjaar and r["mnd"] == mmnd)]
        mnd_alle.append(dict(entry))
        droog_mnd_alle.append(dict(entry))
    mnd_alle.sort(key=lambda x: -x["waarde"])
    droog_mnd_alle.sort(key=lambda x: x["waarde"])
