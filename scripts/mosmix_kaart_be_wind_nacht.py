...
print("MOSMIX ophalen (wind nacht 18-06u België)...")
data_per_day = {}
...
for day, dag_data in data_per_day.items():
    ...
    ax_h.text(0.012, 0.22, "Windkracht nacht (18–06u)  ·  MOS ECMWF/ICON", fontsize=8,
              color="#94a3b8", va="center", transform=ax_h.transAxes)
    ax_h.text(0.988, 0.65,
              f"Wind België – Nacht naar {next_dag_nl} {next_day.day} {nl_maanden[next_day.month]}",
    ...
    leg.text(0.5, 0.94, "Windkracht (18-06u)", fontsize=4.5, weight="bold",
    ...
    fname = f"kaart_be_wind_nacht_{dag_nl.lower()}_{day.strftime('%d%b%Y').lower()}.png"
    plt.savefig(fname, dpi=150, bbox_inches="tight")
    plt.close()
    print(f"Kaart: {fname}")

# Ruim oude kaarten op (max 7, op bestandsdatum)
for oud in sorted(glob.glob("kaart_be_wind_nacht_*.png"), key=os.path.getmtime)[:-10]:
