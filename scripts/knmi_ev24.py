#!/usr/bin/env python3
"""
knmi_ev24.py — Download neerslag en Makkink-referentieverdamping voor AWS-stations.

Voor het neerslagtekort per P13-neerslagstation pairen we elk P13-station aan
een nabije AWS. Rotterdam Airport wordt daarnaast als zelfstandige lokale
droogtereeks bijgehouden. EV24 is pas vanaf ~1957 beschikbaar.
...
AWS_NAMEN = {
    ...
    319: "Westdorpe", 344: "Rotterdam Airport", 350: "Gilze-Rijen", 377: "Ell",
}
...
CACHE_UREN = 2   # in uren, niet dagen: anders haalt een tweede run op dezelfde
                 # dag nooit de nieuw toegevoegde KNMI-dag op
...
def download_station(stn: int) -> str:
    ...
    if os.path.exists(cache_path):
        uren = (datetime.now() - datetime.fromtimestamp(os.path.getmtime(cache_path))).total_seconds() / 3600
        if uren < CACHE_UREN:
            print(f"  Cache ({uren:.1f}u): stn {stn} ({AWS_NAMEN.get(stn, '?')})")
            return cache_path
...
def main():
    ...
    print("=" * 60)
    unieke = sorted(set(P13_TO_AWS.values()) | set(EXTRA_AWS))
