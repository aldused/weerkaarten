...
return async function handle(request,env,ctx){
  ...
  if(request.method==='GET'&&!url.search&&/^\/data_spatial\/ecmwf_ifs\/(latest\.json|\d{4}\/\d{2}\/\d{2}\/(00|06|12|18)00Z\/meta\.json)$/.test(url.pathname)){
  ...
  if(!variables.has(variable)||url.searchParams.get('v')!=='3'||[...url.searchParams].length!==3||bounds.length!==4||!bounds.every(Number.isFinite)||bounds[0]<-26.01||bounds[2]>46.01||bounds[1]<28.99||bounds[3]>73.01||bounds[0]>=bounds[2]||bounds[1]>=bounds[3])return fail('Ongeldige kaartselectie',400);
  const run=Date.parse(match[1].replaceAll('/','-')+'T'+match[2]+':00Z'),time=Date.parse(match[3]+'T'+match[4]+':00Z');
  if(!Number.isFinite(run)||!Number.isFinite(time)||time<run||time>run+360*3600000||run>now())return fail('Ongeldige modeltijd',400);
  const keyURL=new URL(url);keyURL.search=new URLSearchParams({v:'3',variable,bounds:bounds.join(',')}).toString();
  const key=new Request(keyURL),cached=await getCache().match(key);
  if(cached)return deliver(cached,true,`cache;dur=${performance.now()-started}`);
  try{
    const dy=3*180/(2560+.5),ranges=getRanges(grid,[bounds[0]-dy,bounds[1]-dy,bounds[2]+dy,bounds[3]+dy]);
    request.signal.throwIfAborted();
    const source=ROOT+url.pathname,loaded=await readField(source,variable,ranges,request.signal);
    request.signal.throwIfAborted();
    const readEnd=performance.now(),packet=packField(loaded,grid,ranges,bounds,{source:url.pathname,variable});
    const bytes=encodePacket(packet),packEnd=performance.now();
