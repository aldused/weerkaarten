"""
pluim_barendrecht_icon.py — ICON-EPS pluim Barendrecht
...
print("Data ophalen Den Helder ICON-EPS...")
data   = haal_ensemble()
...
header_ax.text(0.012,0.22,f"ICON-EPS · {n_leden} leden · 7 dagen",
          fontsize=8,color="#a8c8e8",va="center",transform=header_ax.transAxes)
header_ax.text(0.988,0.65,"Ensemble pluim – Den Helder (ICON)",
...
leg1 = [
    Line2D([0],[0], color="#27ae60", lw=1, alpha=0.6, label=f"ICON-EPS ({n_leden} leden)"),
    Line2D([0],[0], color="#cc2200", lw=2.5, label="Mediaan"),
    Line2D([0],[0], color="#cc2200", lw=1.5, linestyle="--", label="10°C"),
]
ax1.legend(handles=leg1, loc="upper right", fontsize=8, framealpha=0.9, edgecolor="#cccccc", ncol=3)
...
fig.text(0.98, 0.005, f"© Ed Aldus | Data: ICON-EPS via Open-Meteo | {now_str}",
         fontsize=7, style="italic", ha="right", va="bottom", color="#555555")

fname = "pluim_denhelder_icon.png"
