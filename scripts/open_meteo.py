...
# ---------------------------------------------------------------------------
# Keep-alive-sessie voor bulk-roosters
# ---------------------------------------------------------------------------
# Open-Meteo verdeelt aanvragen over meerdere nodes en die lopen niet gelijk:
# vlak na een modelrun serveert de ene node de nieuwe data en de andere nog de
# oude, en bij `meteofrance_arome_france_hd` bleek een deel van de nodes gewoon
# de 2,5 km-velden van het gewone AROME terug te geven. Een rooster dat met een
# losse requests.post() per batch wordt opgehaald, opent per batch een nieuwe
# verbinding en dus vaak een andere node — het resultaat is een lappendeken van
# twee bronnen, zichtbaar als horizontale strepen precies op de batchgrenzen.
#
# Eén keep-alive-sessie blijft aan dezelfde node hangen (gemeten: 10/10
# identieke antwoorden, tegen 8/2 met losse requests). Gebruik in bulk-scripts:
#
#     from open_meteo import bulk_sessie, herstart_sessie
#     r = bulk_sessie().post(url, json=payload, timeout=120)   # i.p.v. requests.post
#     herstart_sessie()   # bij fout of nodewissel: andere node afdwingen

_SESSIE: Optional[requests.Session] = None


def bulk_sessie() -> requests.Session:
    """Gedeelde sessie; houdt alle batches van één rooster op dezelfde node."""
    global _SESSIE
    if _SESSIE is None:
...
def herstart_sessie() -> None:
    """Verbinding verbreken zodat de load balancer een andere node toewijst."""
    global _SESSIE
    if _SESSIE is not None:
        try:
            _SESSIE.close()
        except Exception:
            pass
