...
(async()=>{
  ...
  try {
    ...
    assert.equal(dataRequests,1,'concepts load immediately without a login');
    assert.equal(await page.locator('#login-gate, #login-password, #logout').count(),0);
    assert.equal(await page.locator('body').evaluate(el=>el.classList.contains('locked')),false);
    assert.equal(await page.locator('.article').count(),2);
    if(payload.demoOnly)assert.match(await page.locator('#alert').innerText(),/Zaterdagdemo/);
    assert.equal(await page.locator('.counter.bad').count(),0);
    await page.locator('#copy-paper').click();
    const clip=await page.evaluate(()=>navigator.clipboard.readText());
    if(payload.demoOnly)assert.match(clip,/DEMO — GEEN ZONDAGSKRANT/);
    ...
    await page.reload();await page.locator('#body-vk_lang').waitFor();await page.locator('[data-paper="ad"]').click();
    ...
    console.log('PASS: direct access without login, five versions, combined copy, word/punctuation checks, persistence, new revision protection, backup download, network failure, stale edition, mobile layout and print text.');
