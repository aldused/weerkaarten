...
self.onmessage=({data:m})=>{
  if(m.type==='field'){fields.set(m.key,{...m,data:{values:m.values},grid:m.packed?.kind==='regular'?createRegularGrid(m.packed.grid):m.packed?createPackedGrid(m.packed):GridFactory.create(m.gridData,m.ranges)});return;}
