#!/usr/bin/env python3
"""
UK Met Office (UKMO Global) via Open-Meteo naar het canvas-formaat van demo_vierluik_neerslag.html.

Gebruik:
    python3 scripts/ukmo_update.py
"""
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--prefix", default="ukmo_om")
    p.add_argument("--model", default="ukmo_seamless")
...
def post_bulk(batch: list[tuple[int, int, float, float]], args: argparse.Namespace) -> list[dict] | None:
    ...
    raise RuntimeError(f"Open-Meteo UKMO call mislukt: {last_err}")
...
def main() -> int:
    ...
    print(f"UKMO: {n_lat}x{n_lon} = {len(coords)} punten, {len(batches)} batches")
    ...
    meta = {
        "model": f"UK Met Office via Open-Meteo ({args.model})",
