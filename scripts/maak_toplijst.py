...
from knmi_api import KnmiQuotaError, knmi_get
...
# De EDR-locatie-endpoint accepteert meerdere, komma-gescheiden stations-ID's.
# Door alle benodigde parameters per batch op te halen blijft de actuele
...
def haal_buienradar() -> dict:
    ...
    Noot: tx/tn/fx/ff zijn momentopnames, geen etmaal-extremen.
    Ze dienen alleen als fallback als KNMI voor dat station helemaal uitvalt.
    RR komt niet meer van Buienradar (rainFallLast24Hour is rollend 24u).
    """
...
def laatste_24_uur_utc() -> str:
    """Rollend interval van exact 24 uur tot het actuele UTC-tijdstip."""
    end = datetime.now(timezone.utc)
...
def _station_id_uit_dekking(dekking: dict) -> str | None:
    """Lees het WIGOS-ID uit een CoverageJSON-dekking."""
    for sleutel in ("eumetnet:locationId", "inspiregloss:Identifier", "id"):
        waarde = dekking.get(sleutel)
        if isinstance(waarde, str) and waarde:
            return waarde
    return None


def haal_bulk_dekkingen(dt_range: str, parameters: str = BULK_PARAMETERS) -> dict:
    """
    Haal meerdere stations in kleine batches op via één EDR-locatiequery.

    Volgens de KNMI EDR-specificatie mag ``location_id`` meerdere,
    komma-gescheiden stations-ID's bevatten. Dit reduceert een toplijstronde
    van circa 235 API-aanvragen naar hooguit zes (dag + rollende 24 uur).
    """
    station_ids = list(STATIONS)
    resultaat = {}
    for begin in range(0, len(station_ids), BULK_BATCH_SIZE):
        batch = station_ids[begin:begin + BULK_BATCH_SIZE]
        locatie_ids = ",".join(batch)
        params = {"datetime": dt_range, "parameter-name": parameters}
        try:
            r = knmi_get(f"{BASE_URL}/locations/{locatie_ids}", params=params, timeout=45)
            r.raise_for_status()
            js = r.json()
        except KnmiQuotaError as exc:
            print(f"  KNMI bulk afgebroken door quotacircuit: {exc}")
            break
        except Exception as exc:
            nummer = begin // BULK_BATCH_SIZE + 1
            print(f"  KNMI bulkbatch {nummer} mislukt; overige batches blijven bruikbaar: {exc}")
            continue
        dekkingen = js.get("coverages") or ([] if js.get("type") != "Coverage" else [js])
        for dekking in dekkingen:
            station_id = _station_id_uit_dekking(dekking)
            if station_id in STATIONS:
                resultaat[station_id] = dekking
    print(f"  KNMI bulk: {len(resultaat)}/{len(STATIONS)} stations ({parameters})")
    return resultaat


def laatste_waarneming_utc(dekkingen: dict) -> datetime | None:
    """Nieuwste geldige tijdstempel uit een verzameling CoverageJSON-dekkingen."""
    nieuwste = None
    for dekking in dekkingen.values():
        tijden = dekking.get("domain", {}).get("axes", {}).get("t", {}).get("values") or []
        for waarde in tijden[-2:]:  # reeksen zijn chronologisch; twee slots vangen een laatste null op
            try:
                moment = datetime.fromisoformat(waarde.replace("Z", "+00:00"))
            except (AttributeError, TypeError, ValueError):
                continue
            if nieuwste is None or moment > nieuwste:
                nieuwste = moment
    return nieuwste
...
def hoogste_anker_uur_uit_dekking(cov: dict) -> dict | None:
    """Bereken het hoogste anker-uur uit één reeds opgehaalde dekking."""
...
def hoogste_anker_uur(station_id: str, dt_range: str) -> dict | None:
    ...
    return hoogste_anker_uur_uit_dekking(js["coverages"][0])

def haal_t10n_uit_dekking(cov: dict) -> tuple:
    """Laagste 10-cm-temperatuur plus lokaal tijdstip uit een 10-minutendekking."""
    t_vals = cov.get("domain", {}).get("axes", {}).get("t", {}).get("values") or []
    vals = to_floats(cov.get("ranges", {}).get("tgn", {}).get("values"))
    return min_valid(vals), tijdstip_van_min(vals, t_vals, LOCAL_TZ)
...
def haal_t10n_knmi(station_id: str, dag: date, dt_range_10min: str = None) -> float | None:
    ...
    # Fallback: 10-minuten API, parameter tgn (minimum op 10 cm over 6 uur)
    ...
    try:
        ...
        if not js2.get("coverages"):
            return None, None
        return haal_t10n_uit_dekking(js2["coverages"][0])
    except Exception:
        return None, None
...
def haal_temp_wind_uit_dekking(cov: dict) -> dict | None:
    """Temperatuur, windstoot en windchill uit één reeds opgehaalde dekking."""
    t_vals = cov.get("domain", {}).get("axes", {}).get("t", {}).get("values") or []
...
def haal_temp_wind(station_id: str, dt_range: str) -> dict | None:
    ...
    js = r.json()
    if not js.get("coverages"): return None
    return haal_temp_wind_uit_dekking(js["coverages"][0])

def haal_neerslag_uit_dekking(cov: dict) -> tuple:
    """Neerslagsom en tijdstip van hoogste intensiteit uit één dekking."""
    t_vals = cov.get("domain", {}).get("axes", {}).get("t", {}).get("values") or []
...
def haal_neerslag(station_id: str, dt_range: str) -> tuple:
    ...
    return haal_neerslag_uit_dekking(js["coverages"][0])

def haal_wind_only_uit_dekking(cov: dict) -> dict | None:
    """Windstootvelden uit een dekking van een station zonder thermometer."""
    t_vals = cov.get("domain", {}).get("axes", {}).get("t", {}).get("values") or []
    ranges = cov.get("ranges", {})
    fx  = to_floats(ranges.get("fx", {}).get("values"))
    fx_val = max_valid(fx)
...
def haal_zon(station_id: str, dt_range: str) -> tuple:
    ...
    js = r.json()
    if not js.get("coverages"): return None, None
    return haal_zon_uit_dekking(js["coverages"][0])
...
def haal_dag(dag: date) -> dict:
    ...
    rr24_range = laatste_24_uur_utc() if is_vandaag else None
    key        = dag.isoformat()
    res        = {"datum": key, "status": "voorlopig", "update": "",
                  "bronstatus": "onbekend", "bronstations": 0,
                  "bron_laatste_waarneming": None, "bronleeftijd_minuten": None,
                  "bronaantallen": {},
                  "max": [], "min": [], "rr": [], "rr24": [], "fx": [], "ff": [], "t10n": [], "sq": [], "gevoels": []}
    print(f"  Ophalen {dag} ({'tot nu' if is_vandaag else 'heel dag'})...")

    # Buienradar is alleen een actuele fallback. De feed mag nooit gebruikt
    # worden om een ontbrekende historische dag met metingen van vandaag te
    # vullen.
    br = haal_buienradar() if is_vandaag else {}

    # Eén gecombineerde EDR-opvraag per batch in plaats van vijf aanvragen per
    # station. Bij quota-uitputting vallen we direct terug op BR/het eerder
    # opgeslagen daguiterste en veroorzaken we geen duizenden extra 403's.
    try:
        dag_dekkingen = haal_bulk_dekkingen(dt_range)
    except Exception as e:
        print(f"  KNMI bulk dag mislukt: {e} → actuele fallback/cache")
        dag_dekkingen = {}

    res["bronstations"] = len(dag_dekkingen)
    nieuwste_bron = laatste_waarneming_utc(dag_dekkingen)
    if nieuwste_bron is not None:
        res["bron_laatste_waarneming"] = nieuwste_bron.isoformat().replace("+00:00", "Z")
        if is_vandaag:
            leeftijd = max(0.0, (datetime.now(timezone.utc) - nieuwste_bron).total_seconds() / 60.0)
            res["bronleeftijd_minuten"] = round(leeftijd, 1)

    rr24_dekkingen = {}
    if rr24_range and dag_dekkingen:
        try:
            rr24_dekkingen = haal_bulk_dekkingen(rr24_range, "rg")
        except Exception as e:
            print(f"  KNMI bulk 24 uur mislukt: {e} → vorige geldige reeks")

    for station_id, naam in STATIONS.items():
        br_st = br.get(naam, {})  # BR-data voor dit station (leeg dict als ontbreekt)
        dekking = dag_dekkingen.get(station_id)

        # ── Temperatuur & wind (KNMI primair, BR fallback) ───────────────────
        try:
            if dekking is None:
                tw = None
            elif station_id in WIND_ONLY_STATIONS:
                tw = haal_wind_only_uit_dekking(dekking)
            else:
                tw = haal_temp_wind_uit_dekking(dekking)
        ...
        # ── T10N: Buienradar primair (geen tijdstip), KNMI-bulk als fallback ────
        gt = br_st.get("t10n")
        if gt is not None:
            res["t10n"].append((gt, naam, None))  # BR heeft geen tijdstip
        elif dekking is not None:
            try:
                t10n_knmi, t10n_t = haal_t10n_uit_dekking(dekking)
        ...
        try:
            ff_anker = hoogste_anker_uur_uit_dekking(dekking) if dekking is not None else None
        ...
        # ── Zon (alleen KNMI) ─────────────────────────────────────────────────
        try:
            sq, sq_t = haal_zon_uit_dekking(dekking) if dekking is not None else (None, None)
            if sq is not None: res["sq"].append((sq, naam, sq_t))
        except Exception as e:
            print(f"    Zon fout {naam}: {e}")

        # ── Neerslag (alleen KNMI — BR rainFallLast24Hour is rollend 24u) ──────
        try:
            mm, rr_t = haal_neerslag_uit_dekking(dekking) if dekking is not None else (None, None)
            if mm is not None:
                res["rr"].append((mm, naam, rr_t))
        except Exception as e:
            print(f"    Neerslag fout {naam}: {e}")

        # ── Rollende neerslagsom over exact de afgelopen 24 uur (alleen vandaag) ─
        if rr24_range:
            try:
                rr24_dekking = rr24_dekkingen.get(station_id)
                mm24, rr24_t = (haal_neerslag_uit_dekking(rr24_dekking)
                                if rr24_dekking is not None else (None, None))
                if mm24 is not None:
                    res["rr24"].append((mm24, naam, rr24_t))
            except Exception as e:
                print(f"    Neerslag 24 uur fout {naam}: {e}")

    res["max"]    = sorted(res["max"],  reverse=True)
    res["min"]    = sorted(res["min"])
    res["rr"]     = sorted(res["rr"],   reverse=True)
    res["rr24"]   = sorted(res["rr24"], reverse=True)
    ...
    res["bronaantallen"] = {
        sleutel: len({rij[1] for rij in res[sleutel] if len(rij) >= 2})
        for sleutel in ("max", "min", "fx", "ff", "rr", "rr24", "sq", "t10n")
    }
    minimaal_vers = max(5, int(len(STATIONS) * 0.8))
    genoeg_stations = len(dag_dekkingen) >= minimaal_vers
    bron_te_oud = is_vandaag and (
        res["bronleeftijd_minuten"] is None or res["bronleeftijd_minuten"] > 45
    )
    vereiste_aantallen = {"max": 25, "min": 25, "fx": 25, "ff": 25, "rr": 20, "sq": 20}
    if is_vandaag:
        vereiste_aantallen["rr24"] = 20
    genoeg_per_metriek = all(
        res["bronaantallen"].get(sleutel, 0) >= minimum
        for sleutel, minimum in vereiste_aantallen.items()
    )
    if bron_te_oud:
        res["bronstatus"] = "stale"
    elif genoeg_stations and genoeg_per_metriek:
        res["bronstatus"] = "ok"
    else:
        res["bronstatus"] = "degraded"

    if res["bronstatus"] == "ok":
    ...
    print(f"    RR24 top3: {res['rr24'][:3]}")
    return res

def voeg_daguitersten_samen(oude_rijen, nieuwe_rijen, richting: str):
    """
    Bewaar per station het echte daguiterste over opeenvolgende updates.

    Dit voorkomt dat een tijdelijke KNMI-storing een eerder gemeten TX, TN,
    windstoot of dagsom vervangt door een actuele Buienradar-momentopname.
    Bij een gelijke waarde krijgt de nieuwste rij voorrang, zodat het tijdstip
    zo actueel mogelijk blijft.
    """
    per_station = {}
    for rijen in (oude_rijen or [], nieuwe_rijen or []):
        for rij in rijen:
            ...
            try:
                ...
                continue
            naam = rij[1]
            huidig = per_station.get(naam)
            beter = (huidig is None or
                     (richting == "max" and waarde >= float(huidig[0])) or
                     (richting == "min" and waarde <= float(huidig[0])))
            if beter:
                per_station[naam] = list(rij)
    return sorted(per_station.values(), key=lambda item: float(item[0]), reverse=(richting == "max"))


def voeg_ontbrekende_stations_toe(oude_rijen, nieuwe_rijen, richting: str):
    """Vul alleen ontbrekende stations aan zonder nieuwe rollende waarden te overschrijven."""
    resultaat = [list(rij) for rij in (nieuwe_rijen or []) if isinstance(rij, (list, tuple))]
    aanwezige_stations = {rij[1] for rij in resultaat if len(rij) >= 2}
    for rij in oude_rijen or []:
        if isinstance(rij, (list, tuple)) and len(rij) >= 2 and rij[1] not in aanwezige_stations:
            resultaat.append(list(rij))
    return sorted(resultaat, key=lambda item: float(item[0]), reverse=(richting == "max"))
...
def main():
    ...
    # Aantal dagen met echte data in het ingeladen bestand. Hiermee bewaken we
    # dat een run nooit bestaande data wegschrijft.
    geladen_gevuld = sum(
        1 for v in resultaten.values() if isinstance(v, dict) and v.get("max")
    )

    # Zet alle dagen vóór vandaag op definitief
    for key, dag_data in resultaten.items():
        if key < vandaag.isoformat():
            if dag_data.get("status") == "voorlopig":
                dag_data["status"] = "definitief"
            # rr24 is een momentopname vanaf het update-uur en hoort daarom
            # alleen bij de actuele dag, niet in de historische dagkeuze.
            dag_data.pop("rr24", None)
    ...
    # Op de eerste run van een nieuwe dag bestaat er nog geen betrouwbare
    # momentopname om op terug te vallen. Publiceer dan nooit een gedeeltelijke
    # dag die door quota-/bronproblemen is ontstaan; de bestaande historie
    # blijft ongewijzigd en de shellwrapper slaat ook de upload over.
    bestaande_vandaag = resultaten.get(vandaag_key)
    heeft_bruikbare_vandaag = (
        isinstance(bestaande_vandaag, dict) and bool(bestaande_vandaag.get("max"))
    )
    if nieuwe_dag.get("bronstatus") != "ok" and not heeft_bruikbare_vandaag:
        print(
            "\nOnvoldoende verse brondata voor eerste dagrun; "
            "toplijst.json NIET overschreven"
        )
        return 2

    # Bewaar alle cumulatieve daguitersten. Bij een tijdelijke API-storing
    # mogen eerder gemeten extremen nooit teruglopen of verdwijnen.
    if vandaag_key in resultaten:
        ...
        # rr24 is rollend en mag dus wel dalen. Nieuwe stationswaarden krijgen
        # voorrang; alleen ontbrekende stations komen uit de vorige momentopname.
        nieuwe_dag["rr24"] = voeg_ontbrekende_stations_toe(
            oud.get("rr24", []), nieuwe_dag.get("rr24", []), "max"
        )
        # Presenteer een quota-/bronstoring niet als een verse KNMI-run.
        if nieuwe_dag.get("bronstatus") != "ok":
            nieuwe_dag["update"] = oud.get("update", "")

    resultaten[vandaag_key] = nieuwe_dag

    # Historische dagen: maximaal 2 ontbrekende of eerder leeg opgeslagen dagen
    # per run. Lege cache-items worden bewust opnieuw geprobeerd.
    pogingen = 0
    for dag in alle_dagen:
        ...
        if bestaand is not None:
            print(f"  {dag}: lege cache, opnieuw ophalen")
        herstelde_dag = haal_dag(dag)
        pogingen += 1
        if herstelde_dag.get("bronstatus") == "ok" and len(herstelde_dag.get("max", [])) >= 5:
            herstelde_dag["status"] = "definitief"
            resultaten[key] = herstelde_dag
        else:
            print(f"  {dag}: onvoldoende verse stations, lege cache blijft ongewijzigd")
        if pogingen >= 2:
            break
    ...
    if nieuw_gevuld < geladen_gevuld:
        print(f"\nWAARSCHUWING: {geladen_gevuld - nieuw_gevuld} dag(en) met data zouden verdwijnen "
              f"({geladen_gevuld} -> {nieuw_gevuld}) — toplijst.json NIET overschreven")
        return 2
    elif len(vandaag_data.get("max", [])) >= 5 or len(resultaten) > 1:
        # Atomair schrijven: eerst naar tijdelijk bestand, dan hernoemen.
        tmp = f"{JSON_PATH}.{os.getpid()}.tmp"
        with open(tmp, "w") as f:
            json.dump(resultaten, f)
        os.replace(tmp, JSON_PATH)
        print(f"\ntoplijst.json bijgewerkt ({len(resultaten)} dagen, {nieuw_gevuld} met data)")
    else:
