...
from open_meteo import _load_key, has_key, bulk_sessie, herstart_sessie  # noqa: E402
...
def _post_bulk(latitudes, longitudes, hourly: list[str], model: str = None) -> list:
    ...
    for poging in range(3):
        try:
            r = bulk_sessie().post(url, json=payload, timeout=90)
...
def fetch_direct_radiation(
    ...
    for bi, batch in enumerate(batches):
        lats = [lat for (_, _, lat, _) in batch]
        lons = [lon for (_, _, _, lon) in batch]
        locs = _post_bulk(lats, lons, HOURLY_VARS_STRAL)
        if len(locs) != len(batch):
            print(
                f"  [waarschuwing] direct_radiation batch {bi}: {len(locs)} locaties terug "
                f"voor {len(batch)} gevraagd"
            )
        for li, loc in enumerate(locs):
            if li >= len(batch):
...
def main() -> int:
    ...
    meta_grid_cape = {
        ...
        "missing": "NaN; niet als 0 interpreteren",
        "grid": {
            "n_lat": n_lat,
            "n_lon": n_lon,
            "lat_min": lat_min,
    ...
    meta_grid_stral_direct = {
        "file": "harmonie_data_straling_direct.bin",
        "components": 1,
        "label": "Directe kortgolvige straling (W/m²)",
        "source": "open-meteo knmi_harmonie_arome_europe",
        "grid": {
            "n_lat": n_lat,
            "n_lon": n_lon,
